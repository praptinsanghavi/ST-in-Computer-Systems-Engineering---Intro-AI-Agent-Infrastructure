---
description: ''
knowledge_type: official_documentation
scraped_at: '2026-04-05T21:15:15Z'
section: japi
site: akka-io
source_url: https://doc.akka.io/japi/akka/current/akka/dispatch/UnboundedStablePriorityMailbox$.html
title: UnboundedStablePriorityMailbox$
---

# UnboundedStablePriorityMailbox$

## Content

Package [akka.dispatch](package-summary.html)
## Class UnboundedStablePriorityMailbox$

- java.lang.Object
- - akka.dispatch.UnboundedStablePriorityMailbox$

- ---

```
public class UnboundedStablePriorityMailbox$
extends java.lang.Object
```

- - ### Field Summary
	
	
	
	Fields 
	| Modifier and Type | Field | Description |
	| `static [UnboundedStablePriorityMailbox$](UnboundedStablePriorityMailbox$.html "class in akka.dispatch")` | `[MODULE$](#MODULE$)` | Static reference to the singleton instance of this Scala object. |

	- ### Constructor Summary
	
	
	
	Constructors 
	| Constructor | Description |
	| `[UnboundedStablePriorityMailbox$](#%3Cinit%3E())()` |  |

	- ### Method Summary
	
	
		- ### Methods inherited from class java.lang.Object
		
		
		`clone, equals, finalize, getClass, hashCode, notify, notifyAll, toString, wait, wait, wait`

- - ### Field Detail
	
	
	
		- #### MODULE$
		
		
		
		```
		public static final [UnboundedStablePriorityMailbox$](UnboundedStablePriorityMailbox$.html "class in akka.dispatch") MODULE$
		```
		
		Static reference to the singleton instance of this Scala object.

	- ### Constructor Detail
	
	
	
		- #### UnboundedStablePriorityMailbox$
		
		
		
		```
		public UnboundedStablePriorityMailbox$()
		```

## Related Pages (Internal Links)

- https://doc.akka.io/japi/akka/current/akka/dispatch/UnboundedStablePriorityMailbox$.html
- https://doc.akka.io/japi/akka/current/akka/dispatch/package-summary.html

---
*Source: [https://doc.akka.io/japi/akka/current/akka/dispatch/UnboundedStablePriorityMailbox$.html](https://doc.akka.io/japi/akka/current/akka/dispatch/UnboundedStablePriorityMailbox$.html)*