...
scraped_at: '2026-04-05T21:11:54Z'
section: japi
site: akka-io
source_url: https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.html
...
This extension provides sharding functionality of actors in a cluster.
 ...
 in the cluster, registering the supported entity types with the [`init(akka.cluster.sharding.typed.javadsl.Entity<M, E>)`](#init(akka.cluster.sharding.typed.javadsl.Entity))
 ...
 Messages can also be sent via the [`EntityRef`](EntityRef.html "class in akka.cluster.sharding.typed.javadsl") retrieved with [`entityRefFor(akka.cluster.sharding.typed.javadsl.EntityTypeKey<M>, java.lang.String)`](#entityRefFor(akka.cluster.sharding.typed.javadsl.EntityTypeKey,java.lang.String)),
...
- - ### Nested Class Summary
	...
	| `static class` | `[ClusterSharding.Passivate](ClusterSharding.Passivate.html "class in akka.cluster.sharding.typed.javadsl")<[M](ClusterSharding.Passivate.html "type parameter in ClusterSharding.Passivate")>` | The entity can request passivation by sending the [`ClusterSharding.Passivate`](ClusterSharding.Passivate.html "class in akka.cluster.sharding.typed.javadsl") message  to the `ActorRef[ShardCommand]` that was passed in to the factory method  when creating the entity. |
	| `static class` | `[ClusterSharding.Passivate$](ClusterSharding.Passivate$.html "class in akka.cluster.sharding.typed.javadsl")` |  |
	| `static interface` | `[ClusterSharding.ShardCommand](ClusterSharding.ShardCommand.html "interface in akka.cluster.sharding.typed.javadsl")` | When an entity is created an `ActorRef[ShardCommand]` is passed to the  factory method. |
	...
	| `abstract [ShardCoordinator.ShardAllocationStrategy](../../ShardCoordinator.ShardAllocationStrategy.html "interface in akka.cluster.sharding")` | `[defaultShardAllocationStrategy](#defaultShardAllocationStrategy(akka.cluster.sharding.typed.ClusterShardingSettings))​([ClusterShardingSettings](../ClusterShardingSettings.html "class in akka.cluster.sharding.typed") settings)` | The default `ShardAllocationStrategy` is configured by `least-shard-allocation-strategy` properties. |
	| `abstract <M> [EntityRef](EntityRef.html "class in akka.cluster.sharding.typed.javadsl")<M>` | `[entityRefFor](#entityRefFor(akka.cluster.sharding.typed.javadsl.EntityTypeKey,java.lang.String))​([EntityTypeKey](EntityTypeKey.html "class in akka.cluster.sharding.typed.javadsl")<M> typeKey,  java.lang.String entityId)` | Create an `ActorRef`\-like reference to a specific sharded entity. |
	| `abstract <M> [EntityRef](EntityRef.html "class in akka.cluster.sharding.typed.javadsl")<M>` | `[entityRefFor](#entityRefFor(akka.cluster.sharding.typed.javadsl.EntityTypeKey,java.lang.String,java.lang.String))​([EntityTypeKey](EntityTypeKey.html "class in akka.cluster.sharding.typed.javadsl")<M> typeKey,  java.lang.String entityId,  java.lang.String dataCenter)` | Deprecated. Use Akka Distributed Cluster instead. |
	| `static [ClusterSharding](ClusterSharding.html "class in akka.cluster.sharding.typed.javadsl")` | `[get](#get(akka.actor.typed.ActorSystem))​([ActorSystem](../../../../actor/typed/ActorSystem.html "class in akka.actor.typed")<?> system)` |  |
	| `abstract <M,​E>[ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<E>` | `[init](#init(akka.cluster.sharding.typed.javadsl.Entity))​([Entity](Entity.html "class in akka.cluster.sharding.typed.javadsl")<M,​E> entity)` | Initialize sharding for the given `entity` factory settings. |
	| `abstract [ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<[ClusterSharding.ShardCommand](ClusterSharding.ShardCommand.html "interface in akka.cluster.sharding.typed.javadsl")>` | `[shard](#shard(akka.cluster.sharding.typed.javadsl.EntityTypeKey))​([EntityTypeKey](EntityTypeKey.html "class in akka.cluster.sharding.typed.javadsl")<?> typeKey)` | Access to the `ActorRef` to send `ShardCommand` for a given entity type. |
	| `abstract [ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<[ClusterShardingQuery](../ClusterShardingQuery.html "interface in akka.cluster.sharding.typed")>` | `[shardState](#shardState())()` | Actor for querying Cluster Sharding state |
...
- - ### Constructor Detail
	...
	- ### Method Detail
		...
		```
		public static [ClusterSharding](ClusterSharding.html "class in akka.cluster.sharding.typed.javadsl") get​([ActorSystem](../../../../actor/typed/ActorSystem.html "class in akka.actor.typed")<?> system)
		```
		- #### init
		...
		```
		public abstract <M,​E> [ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<E> init​([Entity](Entity.html "class in akka.cluster.sharding.typed.javadsl")<M,​E> entity)
		```
		...
		```
		public abstract <M> [EntityRef](EntityRef.html "class in akka.cluster.sharding.typed.javadsl")<M> entityRefFor​([EntityTypeKey](EntityTypeKey.html "class in akka.cluster.sharding.typed.javadsl")<M> typeKey,
		...
		Create an `ActorRef`\-like reference to a specific sharded entity.
		 ...
		 Messages sent through this [`EntityRef`](EntityRef.html "class in akka.cluster.sharding.typed.javadsl") will be wrapped in a `ShardingEnvelope` including the
		 ...
		 For in\-depth documentation of its semantics, see [`EntityRef`](EntityRef.html "class in akka.cluster.sharding.typed.javadsl").
		- #### entityRefFor
		...
		```
		public abstract [ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<[ClusterShardingQuery](../ClusterShardingQuery.html "interface in akka.cluster.sharding.typed")> shardState()
		```
		...
		```
		public abstract [ActorRef](../../../../actor/typed/ActorRef.html "interface in akka.actor.typed")<[ClusterSharding.ShardCommand](ClusterSharding.ShardCommand.html "interface in akka.cluster.sharding.typed.javadsl")> shard​([EntityTypeKey](EntityTypeKey.html "class in akka.cluster.sharding.typed.javadsl")<?> typeKey)
		```
		
		Access to the `ActorRef` to send `ShardCommand` for a given entity type. For example
		 [`ClusterSharding.Passivate`](ClusterSharding.Passivate.html "class in akka.cluster.sharding.typed.javadsl") can be sent to this `ActorRef`. Note that this `ActorRef`
		 is also available in the [`EntityContext`](EntityContext.html "class in akka.cluster.sharding.typed.javadsl"). The entity type must first be initialized
		...
		```
		public abstract [ShardCoordinator.ShardAllocationStrategy](../../ShardCoordinator.ShardAllocationStrategy.html "interface in akka.cluster.sharding") defaultShardAllocationStrategy​([ClusterShardingSettings](../ClusterShardingSettings.html "class in akka.cluster.sharding.typed") settings)
		```
...
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/ClusterShardingQuery.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/ClusterShardingSettings.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.Passivate$.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.Passivate.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.ShardCommand.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/Entity.html
- https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/EntityContext.html
...
---
*Source: [https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.html](https://doc.akka.io/japi/akka/current/akka/cluster/sharding/typed/javadsl/ClusterSharding.html)*