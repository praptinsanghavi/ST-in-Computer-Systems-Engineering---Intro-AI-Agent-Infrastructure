---
description: Local and remote Akka Actor references, locating Actors, Actor paths
  and addresses.
knowledge_type: official_documentation
scraped_at: '2026-04-06T14:04:54Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/general/addressing.html
title: Actor References, Paths and Addresses • Akka Documentation
---

# Actor References, Paths and Addresses • Akka Documentation

> **Summary:** Local and remote Akka Actor references, locating Actors, Actor paths and addresses.
...
An actor reference is a subtype of [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef"), whose foremost purpose is to support sending messages to the actor it represents. Each actor has access to its canonical (local) reference through the [`ActorContext.self`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/typed/javadsl/ActorContext.html#getSelf() "akka.actor.typed.javadsl.ActorContext")[`ActorContext.self`](https://doc.akka.io/api/akka-core/2.6/akka/actor/typed/scaladsl/ActorContext.html#self:akka.actor.typed.ActorRef[T] "akka.actor.typed.scaladsl.ActorContext") field; this reference can be included in messages to other actors to get replies back.
...
- There are several special types of actor references which behave like local actor references for all practical purposes:
	- `PromiseActorRef` is the special representation of a [`Promise`](http://www.scala-lang.org/api/2.13.8/scala/concurrent/Promise.html "scala.concurrent.Promise") for the purpose of being completed by the response from an actor. `akka.pattern.ask` creates this actor reference.
...
- And then there are some one\-off internal implementations which you should never really see:
	...
	- The first logging service started before actually firing up actor creation facilities is a fake actor reference which accepts log events and prints them directly to standard output; it is [`Logging.StandardOutLogger`](https://doc.akka.io/api/akka-core/2.6/akka/event/Logging$$StandardOutLogger.html "akka.event.Logging.StandardOutLogger")[`Logging.StandardOutLogger`](https://doc.akka.io/japi/akka-core/2.6/akka/event/Logging.StandardOutLogger.html "akka.event.Logging.StandardOutLogger").
...
"akka://[[email protected]](/cdn-cgi/l/email-protection):5678/user/service-b" // remote
...
There are two general categories to how actor references may be obtained: by [creating actors](../typed/actor-lifecycle.html#creating-actors) or by looking them up through the [Receptionist](../typed/actor-discovery.html#receptionist).

## Actor Reference and Path Equality

Equality of [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef") match the intention that an [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef") corresponds to the target actor incarnation. Two actor references are compared equal when they have the same path and point to the same actor incarnation. A reference pointing to a terminated actor does not compare equal to a reference pointing to another (re\-created) actor with the same path. Note that a restart of an actor caused by a failure still means that it is the same actor incarnation, i.e. a restart is not visible for the consumer of the [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef").

If you need to keep track of actor references in a collection and do not care about the exact actor incarnation you can use the [`ActorPath`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorPath.html "akka.actor.ActorPath")[`ActorPath`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorPath.html "akka.actor.ActorPath") as key, because the identifier of the target actor is not taken into account when comparing actor paths.
...
- `"/user"` is the guardian actor for all user\-created top\-level actors; actors created using [`ActorSystem.actorOf`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRefFactory.html#actorOf(props:akka.actor.Props):akka.actor.ActorRef "akka.actor.ActorRefFactory")[`ActorSystem.actorOf`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRefFactory.html#actorOf(akka.actor.Props) "akka.actor.ActorRefFactory") are found below this one.
...
- `"/temp"` is the guardian for all short\-lived system\-created actors, e.g. those which are used in the implementation of [`ActorRef.ask`](https://doc.akka.io/api/akka-core/2.6/akka/pattern/AskableActorRef.html#ask(message:Any)(implicittimeout:akka.util.Timeout,implicitsender:akka.actor.ActorRef):scala.concurrent.Future[Any] "akka.pattern.AskableActorRef")[`Patterns.ask`](https://doc.akka.io/japi/akka-core/2.6/akka/pattern/Patterns.html#ask(akka.actor.ActorRef,java.lang.Object,java.time.Duration) "akka.pattern.Patterns").
...
### Example 1: Actor Path Anchors

```text
"akka://my-sys/user/service-a/worker1"               // purely local
"akka://[email protected]:5678/user/service-b" // remote
```
...
- https://doc.akka.io/api/akka-core/2.6/akka/pattern/AskableActorRef.html
...
- https://doc.akka.io/japi/akka-core/2.6/akka/event/Logging.StandardOutLogger.html
- https://doc.akka.io/japi/akka-core/2.6/akka/pattern/Patterns.html
...
---
*Source: [https://doc.akka.io/libraries/akka-core/2.6/general/addressing.html](https://doc.akka.io/libraries/akka-core/2.6/general/addressing.html)*