---
description: The Akka Cluster node membership service, manages dynamic member states
  and lifecycle with no external infrastructure needed.
knowledge_type: official_documentation
scraped_at: '2026-04-06T14:15:18Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/typed/cluster-membership.html
title: Cluster Membership Service • Akka Documentation
---

# Cluster Membership Service • Akka Documentation

> **Summary:** The Akka Cluster node membership service, manages dynamic member states and lifecycle with no external infrastructure needed.

## Content

# Cluster Membership Service

The core of Akka Cluster is the cluster membership, to keep track of what nodes are part of the cluster and their health. Cluster membership is communicated using [gossip](cluster-concepts.html#gossip) and [failure detection](cluster-concepts.html#failure-detector).

There are several [Higher level Cluster tools](cluster.html#higher-level-cluster-tools) that are built on top of the cluster membership service.
...
The cluster membership state is a specialized [CRDT](https://hal.inria.fr/file/index/docid/555588/filename/techreport.pdf), which means that it has a monotonic merge function. When concurrent changes occur on different nodes the updates can always be merged and converge to the same end result.
...
A node is introduced to the cluster by invoking the `join` action which puts the node in the `joining` state. Once all nodes have seen that the new node is joining (through [gossip convergence](cluster-concepts.html#gossip-convergence)) the [`leader`](cluster-concepts.html#leader) will set the member state to `up`.

If a node is leaving the cluster in a safe, expected manner, for example through [coordinated shutdown](../coordinated-shutdown.html), it invokes the `leave` action which switches it to the `leaving` state. Once the leader sees the convergence on the node in the `leaving` state, the leader will then move it to `exiting`. Once all nodes have seen the exiting state (convergence) the `leader` will remove the node from the cluster, marking it as `removed`.

If a node is `unreachable` then gossip convergence is not possible and therefore most `leader` actions are impossible (for instance, allowing a node to become a part of the cluster). To be able to move forward, the node must become `reachable` again or the node must be explicitly “downed”. This is required because the state of an unreachable node is unknown and the cluster cannot know if the node has crashed or is only temporarily unreachable because of network issues or GC pauses. See the section about [User Actions](#user-actions) below for ways a node can be downed.
...
To use this feature use `Cluster(system).prepareForFullClusterShutdown()` in classic or [`PrepareForFullClusterShutdown`](https://doc.akka.io/api/akka-core/2.6/akka/cluster/typed/PrepareForFullClusterShutdown.html "akka.cluster.typed.PrepareForFullClusterShutdown")[`PrepareForFullClusterShutdown`](https://doc.akka.io/japi/akka-core/2.6/akka/cluster/typed/PrepareForFullClusterShutdown.html "akka.cluster.typed.PrepareForFullClusterShutdown") in typed.
...
- **leave** \- tell a node to leave the cluster gracefully, normally triggered by ActorSystem or JVM shutdown through [coordinated shutdown](../coordinated-shutdown.html)
- **down** \- mark a node as down. This action is required to remove crashed nodes (that did not ‘leave’) from the cluster. It can be triggered manually, through [Cluster HTTP Management](https://doc.akka.io/libraries/akka-management/current/cluster-http-management.html#put-cluster-members-address-responses), or automatically by a [downing provider](cluster.html#downing) like [Split Brain Resolver](../split-brain-resolver.html)
...
- https://doc.akka.io/api/akka-core/2.6/akka/cluster/typed/PrepareForFullClusterShutdown.html
- https://doc.akka.io/japi/akka-core/2.6/akka/cluster/typed/PrepareForFullClusterShutdown.html
...
- https://doc.akka.io/libraries/akka-core/2.6/typed/cluster-concepts.html
...
---
*Source: [https://doc.akka.io/libraries/akka-core/2.6/typed/cluster-membership.html](https://doc.akka.io/libraries/akka-core/2.6/typed/cluster-membership.html)*