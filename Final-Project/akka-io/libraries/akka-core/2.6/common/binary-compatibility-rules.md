---
description: Binary compatibility across Akka versions.
knowledge_type: official_documentation
scraped_at: '2026-04-06T03:14:46Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/common/binary-compatibility-rules.html
title: Binary Compatibility Rules • Akka Documentation
---

# Binary Compatibility Rules • Akka Documentation

> **Summary:** Binary compatibility across Akka versions.
...
Because of this approach applications can upgrade to the latest version of Akka even when [intermediate satellite projects are not yet upgraded](../project/downstream-upgrade-strategy.html)

## Binary compatibility rules explained

Binary compatibility is maintained between:

- **minor** and **patch** versions \- please note that the meaning of “minor” has shifted to be more restrictive with Akka `2.4.0`, read [Change in versioning scheme](binary-compatibility-rules.html#24versioningchange) for details.

Binary compatibility is **NOT** maintained between:

- **major** versions
- any versions of **may change** modules – read [Modules marked “May Change”](may-change.html) for details
- a few notable exclusions explained below

Specific examples (please read [Change in versioning scheme](binary-compatibility-rules.html#24versioningchange) to understand the difference in “before 2\.4 era” and “after 2\.4 era”):
...
- all [may change](may-change.html) modules \- which by definition are subject to rapid iteration and change. Read more about that in [Modules marked “May Change”](may-change.html)
...
Historically, Akka has been following the Java or Scala style of versioning in which the first number would mean “**epoch**”, the second one would mean **major**, and third be the **minor**, thus: `epoch.major.minor` (versioning scheme followed until and during `2.3.x`).

**Currently**, since Akka `2.4.0`, the new versioning applies which is closer to semantic versioning many have come to expect, in which the version number is deciphered as `major.minor.patch`. This also means that Akka `2.5.x` is binary compatible with the `2.4` series releases (with the exception of “may change” APIs).
...
Modules that are released together under the Akka project are intended to be upgraded together. For example, it is not legal to mix Akka Actor `2.6.2` with Akka Cluster `2.6.5` even though “Akka `2.6.2`” and “Akka `2.6.5`” *are* binary compatible. 
...
The fix is typically to pick the highest Akka version, and add explicit dependencies to your project as needed. For example, in the example above you might want to add `akka-persistence-query` dependency for 2\.6\.6\.
...
The warning includes a full list of Akka runtime dependencies in the classpath, and the version detected. You can use that information to include an explicit list of Akka artifacts you depend on into your build. If you use Maven or Gradle, you can include the [Akka Maven BOM](../typed/guide/modules.html#actor-library) (bill of materials) to help you keep all the versions of your Akka dependencies in sync. 

## The meaning of “may change”

**May change** is used in module descriptions and docs in order to signify that the API that they contain is subject to change without any prior warning and is not covered by the binary compatibility promise. Read more in [Modules marked “May Change”](may-change.html).
...
When browsing the source code and/or looking for methods available to be called, especially from Java which does not have as rich of an access protection system as Scala has, you may sometimes find methods or classes annotated with the `/** INTERNAL API */` comment or the [`@InternalApi`](https://doc.akka.io/japi/akka-core/2.6/akka/annotation/InternalApi.html "akka.annotation.InternalApi") annotation. 
...
Side\-note on JVM representation details of the Scala `private[akka]` pattern that Akka is using extensively in its internals: Such methods or classes, which act as “accessible only from the given package” in Scala, are compiled down to `public` (!) in raw Java bytecode. The access restriction, that Scala understands is carried along as metadata stored in the classfile. Thus, such methods are safely guarded from being accessed from Scala, however Java users will not be warned about this fact by the `javac` compiler. Please be aware of this and do not call into Internal APIs, as they are subject to change without any warning.
...
- [`@ApiMayChange`](https://doc.akka.io/japi/akka-core/2.6/akka/annotation/ApiMayChange.html "akka.annotation.ApiMayChange") – which marks APIs which are known to be not fully stable yet. Read more in [Modules marked “May Change”](may-change.html)
- [`@DoNotInherit`](https://doc.akka.io/japi/akka-core/2.6/akka/annotation/DoNotInherit.html "akka.annotation.DoNotInherit") – which marks APIs that are designed under a closed\-world assumption, and thus must not be extended outside Akka itself (or such code will risk facing binary incompatibilities). E.g. an interface may be marked using this annotation, and while the type is public, it is not meant for extension by user\-code. This allows adding new methods to these interfaces without risking to break client code. Examples of such API are the [`FlowOps`](https://doc.akka.io/api/akka-core/2.6/akka/stream/scaladsl/FlowOps.html "akka.stream.scaladsl.FlowOps") trait or the Akka HTTP domain model.
...
Akka uses the Lightbend maintained [MiMa](https://github.com/lightbend/mima), for enforcing binary compatibility is kept where it was promised.
...
### Example 1: Binary compatibility rules explained
...
### Example 2: Mixed versioning is not allowed
...
- https://doc.akka.io/api/akka-core/2.6/akka/stream/scaladsl/FlowOps.html
- https://doc.akka.io/japi/akka-core/2.6/akka/annotation/ApiMayChange.html
- https://doc.akka.io/japi/akka-core/2.6/akka/annotation/DoNotInherit.html
- https://doc.akka.io/japi/akka-core/2.6/akka/annotation/InternalApi.html
...
- https://doc.akka.io/libraries/akka-core/2.6/project/downstream-upgrade-strategy.html
- https://doc.akka.io/libraries/akka-core/2.6/project/index.html
- https://doc.akka.io/libraries/akka-core/2.6/project/scala3.html
- https://doc.akka.io/libraries/akka-core/2.6/typed/guide/modules.html

---
*Source: [https://doc.akka.io/libraries/akka-core/2.6/common/binary-compatibility-rules.html](https://doc.akka.io/libraries/akka-core/2.6/common/binary-compatibility-rules.html)*