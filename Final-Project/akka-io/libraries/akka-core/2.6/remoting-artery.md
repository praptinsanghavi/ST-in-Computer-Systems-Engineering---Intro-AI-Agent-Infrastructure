---
description: Details about the underlying remoting module for Akka Cluster.
knowledge_type: official_documentation
scraped_at: '2026-04-06T03:14:51Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/remoting-artery.html
title: Artery Remoting • Akka Documentation
---

# Artery Remoting • Akka Documentation

> **Summary:** Details about the underlying remoting module for Akka Cluster.
...
If migrating from classic remoting see [what’s new in Artery](remoting-artery.html#what-is-new-in-artery)
...
One option is to use Artery with Aeron, see [Selecting a transport](remoting-artery.html#selecting-a-transport). The Aeron dependency needs to be explicitly added if using the `aeron-udp` transport:
...
The example above only illustrates the bare minimum of properties you have to add to enable remoting. All settings are described in [Remote Configuration](remoting-artery.html#remote-configuration-artery).
...
Note
This page describes the remoting subsystem, codenamed *Artery* that has replaced the [classic remoting implementation](remoting.html).

Remoting enables Actor systems on different hosts or JVMs to communicate with each other. By enabling remoting the system will start listening on a provided network address and also gains the ability to connect to other systems through the network. From the application’s perspective there is no API difference between local or remote systems, [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef") instances that point to remote systems look exactly the same as local ones: they can be sent messages to, watched, etc. Every `ActorRef` contains hostname and port information and can be passed around even on the network. This means that on a network every `ActorRef` is a unique identifier of an actor on that network.
...
- `tcp` \- Based on [Akka Streams TCP](stream/stream-io.html#streaming-tcp) (default if other not configured)
- `tls-tcp` \- Same as `tcp` with encryption using [Akka Streams TLS](stream/stream-io.html#tls)
...
Note
[Rolling update](additional/rolling-updates.html) is not supported when changing from one transport to another.

## Migrating from classic remoting

See [migrating from classic remoting](project/migration-guide-2.5.x-2.6.x.html#classic-to-artery)

## Canonical address

In order for remoting to work properly, where each system can send messages to any other system on the same network (for example a system forwards a message to a third system, and the third replies directly to the sender system) it is essential for every system to have a *unique, globally reachable* address and port. This address is part of the unique name of the system and will be used by other systems to open a connection to it and send messages. This means that if a host has multiple names (different DNS records pointing to the same IP address) then only one of these can be *canonical*. If a message arrives to a system but it contains a different hostname than the expected canonical name then the message will be dropped. If multiple names for a system would be allowed, then equality checks among [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef") instances would no longer to be trusted and this would violate the fundamental assumption that an actor has a globally unique reference on a given network. As a consequence, this also means that localhost addresses (e.g. *127\.0\.0\.1*) cannot be used in general (apart from local development) since they are not unique addresses in a real network.

In cases, where Network Address Translation (NAT) is used or other network bridging is involved, it is important to configure the system so that it understands that there is a difference between his externally visible, canonical address and between the host\-port pair that is used to listen for connections. See [Akka behind NAT or in a Docker container](remoting-artery.html#remote-configuration-nat-artery) for details.

## Acquiring references to remote actors

In order to communicate with an actor, it is necessary to have its [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef"). In the local case it is usually the creator of the actor (the caller of `actorOf()`) is who gets the `ActorRef` for an actor that it can then send to other actors. In other words:

- An Actor can get a remote Actor’s reference by receiving a message from it (as it’s available as [`sender()`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Actor.html#sender():akka.actor.ActorRef "akka.actor.Actor")[`getSender()`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/AbstractActor.html#getSender() "akka.actor.AbstractActor") then), or inside of a remote message (e.g. *PleaseReply(message: String, remoteActorRef: ActorRef)*)

Alternatively, an actor can look up another located at a known path using [`ActorSelection`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection"). These methods are available even in remoting enabled systems:

- Remote Lookup : used to look up an actor on a remote node with [`actorSelection(path)`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRefFactory.html#actorSelection(path:String):akka.actor.ActorSelection "akka.actor.ActorRefFactory")[`actorSelection(path)`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRefFactory.html#actorSelection(java.lang.String) "akka.actor.ActorRefFactory")
- Remote Creation : used to create an actor on a remote node with [`actorOf(Props(...), actorName)`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRefFactory.html#actorOf(props:akka.actor.Props,name:String):akka.actor.ActorRef "akka.actor.ActorRefFactory")[`actorOf(Props(...), actorName)`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRefFactory.html#actorOf(akka.actor.Props,java.lang.String) "akka.actor.ActorRefFactory")

In the next sections the two alternatives are described in detail.

### Looking up Remote Actors

[`actorSelection(path)`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRefFactory.html#actorSelection(path:String):akka.actor.ActorSelection "akka.actor.ActorRefFactory")[`actorSelection(path)`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRefFactory.html#actorSelection(java.lang.String) "akka.actor.ActorRefFactory") will obtain an [`ActorSelection`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection") to an Actor on a remote node, e.g.:

Scala

```
val selection =
  context.actorSelection("akka://[[email protected]](/cdn-cgi/l/email-protection):25520/user/actorName")
...
To acquire an [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef") for an [`ActorSelection`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorSelection.html "akka.actor.ActorSelection") you need to send a message to the selection and use the [`sender()`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Actor.html#sender():akka.actor.ActorRef "akka.actor.Actor")[`getSender()`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/AbstractActor.html#getSender() "akka.actor.AbstractActor") reference of the reply from the actor. There is a built\-in [`Identify`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Identify.html "akka.actor.Identify")[`Identify`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/Identify.html "akka.actor.Identify") message that all Actors will understand and automatically reply to with a [`ActorIdentity`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorIdentity.html "akka.actor.ActorIdentity")[`ActorIdentity`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorIdentity.html "akka.actor.ActorIdentity") message containing the `ActorRef`. This can also be done with the [`resolveOne`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorSelection.html#resolveOne(timeout:scala.concurrent.duration.FiniteDuration):scala.concurrent.Future[akka.actor.ActorRef] "akka.actor.ActorSelection")[`resolveOne`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorSelection.html#resolveOne(java.time.Duration) "akka.actor.ActorSelection") method of the `ActorSelection`, which returns a [`Future`](http://www.scala-lang.org/api/2.13.8/scala/concurrent/Future.html "scala.concurrent.Future")[`CompletionStage`](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/concurrent/CompletionStage.html "java.util.concurrent.CompletionStage") of the matching `ActorRef`.
...
An [`ActorSystem`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorSystem.html "akka.actor.ActorSystem")[`ActorSystem`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorSystem.html "akka.actor.ActorSystem") should not be exposed via Akka Remote (Artery) over plain Aeron/UDP or TCP to an untrusted network (e.g. Internet). It should be protected by network security, such as a firewall. If that is not considered as enough protection [TLS with mutual authentication](remoting-artery.html#remote-tls) should be enabled.
...
Mutual authentication between TLS peers is enabled by default. Mutual authentication means that the the passive side (the TLS server side) of a connection will also request and verify a certificate from the connecting peer. Without this mode only the client side is requesting and verifying certificates. While Akka is a peer\-to\-peer technology, each connection between nodes starts out from one side (the “client”) towards the other (the “server”).
...
As soon as an actor system can connect to another remotely, it may in principle send any possible message to any actor contained within that remote system. One example may be sending a [`PoisonPill`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill")[`PoisonPill`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill") to the system guardian, shutting that system down. This is not always desired, and it can be disabled with the following setting:
...
This disallows sending of system messages (actor life\-cycle commands, DeathWatch, etc.) and any message extending [`PossiblyHarmful`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful")[`PossiblyHarmful`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful") to the system on which this flag is set. Should a client send them nonetheless they are dropped and logged (at DEBUG level in order to reduce the possibilities for a denial of service attack). `PossiblyHarmful` covers the predefined messages like [`PoisonPill`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill")[`PoisonPill`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill") and [`Kill`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Kill.html "akka.actor.Kill")[`Kill`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/Kill.html "akka.actor.Kill"), but it can also be added as a marker trait to user\-defined messages.

Warning
Untrusted mode does not give full protection against attacks by itself. It makes it slightly harder to perform malicious or unintended actions but it should be noted that [Java serialization](serialization.html#java-serialization) should still not be enabled. Additional protection can be achieved when running in an untrusted network by network security (e.g. firewalls) and/or enabling [TLS with mutual authentication](remoting-artery.html#remote-tls).
...
- remote DeathWatch
- [`system.stop()`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRefFactory.html#stop(actor:akka.actor.ActorRef):Unit "akka.actor.ActorRefFactory")[`system.stop()`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRefFactory.html#stop(akka.actor.ActorRef) "akka.actor.ActorRefFactory"), [`PoisonPill`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill")[`PoisonPill`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PoisonPill.html "akka.actor.PoisonPill"), [`Kill`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Kill.html "akka.actor.Kill")[`Kill`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/Kill.html "akka.actor.Kill")
- sending any message which extends from the [`PossiblyHarmful`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful")[`PossiblyHarmful`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful") marker interface, which includes [`Terminated`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Terminated.html "akka.actor.Terminated")[`Terminated`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/Terminated.html "akka.actor.Terminated")
- messages sent with actor selection, unless destination defined in `trusted-selection-paths`.

Note
Enabling the untrusted mode does not remove the capability of the client to freely choose the target of its message sends, which means that messages not prohibited by the above rules can be sent to any actor in the remote system. It is good practice for a client\-facing system to only contain a well\-defined set of entry point actors, which then forward requests (possibly after performing validation) to another actor system containing the actual worker actors. If messaging between these two server\-side systems is done using local [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef") (they can be exchanged safely between actor systems within the same JVM), you can restrict the messages on this interface by marking them [`PossiblyHarmful`](https://doc.akka.io/api/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful")[`PossiblyHarmful`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/PossiblyHarmful.html "akka.actor.PossiblyHarmful") so that a client cannot forge them.

## Quarantine

Akka remoting is using TCP or Aeron as underlying message transport. Aeron is using UDP and adds among other things reliable delivery and session semantics, very similar to TCP. This means that the order of the messages are preserved, which is needed for the [Actor message ordering guarantees](general/message-delivery-reliability.html#message-ordering). Under normal circumstances all messages will be delivered but there are cases when messages may not be delivered to the destination:
...
In short, Actor message delivery is “at\-most\-once” as described in [Message Delivery Reliability](general/message-delivery-reliability.html)
...
Watching a remote actor is API wise not different than watching a local actor, as described in [Lifecycle Monitoring aka DeathWatch](actors.html#deathwatch). However, it is important to note, that unlike in the local case, remoting has to handle when a remote actor does not terminate in a graceful way sending a system message to notify the watcher actor about the event, but instead being hosted on a system which stopped abruptly (crashed). These situations are handled by the built\-in failure detector.

### Failure Detector

Under the hood remote death watch uses heartbeat messages and a failure detector to generate [`Terminated`](https://doc.akka.io/api/akka-core/2.6/akka/actor/Terminated.html "akka.actor.Terminated")[`Terminated`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/Terminated.html "akka.actor.Terminated") message from network failures and JVM crashes, in addition to graceful termination of watched actor.
...
In the [Remote Configuration](remoting-artery.html#remote-configuration-artery) you can adjust the `akka.remote.watch-failure-detector.threshold` to define when a *phi* value is considered to be a failure.
...
To be able to survive sudden abnormalities, such as garbage collection pauses and transient network failures the failure detector is configured with a margin, `akka.remote.watch-failure-detector.acceptable-heartbeat-pause`. You may want to adjust the [Remote Configuration](remoting-artery.html#remote-configuration-artery) of this depending on you environment. This is how the curve looks like for `acceptable-heartbeat-pause` configured to 3 seconds.
...
Artery introduces a new serialization mechanism which allows the [`ByteBufferSerializer`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/ByteBufferSerializer.html "akka.serialization.ByteBufferSerializer")[`ByteBufferSerializer`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/ByteBufferSerializer.html "akka.serialization.ByteBufferSerializer") to directly write into a shared [`java.nio.ByteBuffer`](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/nio/ByteBuffer.html "java.nio.ByteBuffer") instead of being forced to allocate and return an `Array[Byte]` for each serialized message. For high\-throughput messaging this API change can yield significant performance benefits, so we recommend changing your serializers to use this new mechanism.
...
As the new feature only changes how bytes are read and written, and the rest of the serialization infrastructure remained the same, we recommend reading the [Serialization](serialization.html) documentation first.
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-actor/src/main/scala/akka/serialization/Serializer.scala#L202-L216 "Go to snippet source")trait ByteBufferSerializer {
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/java/jdocs/actor/ByteBufferSerializerDocTest.java#L63-L72 "Go to snippet source")interface ByteBufferSerializer {
  /** Serializes the given object into the `ByteBuffer`. */
...
Implementing a serializer for Artery is therefore as simple as implementing this interface, and binding the serializer as usual (which is explained in [Serialization](serialization.html)).

Implementations should typically extend [`SerializerWithStringManifest`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html "akka.serialization.SerializerWithStringManifest")[`SerializerWithStringManifest`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html "akka.serialization.SerializerWithStringManifest") and in addition to the `ByteBuffer` based [`toBinary`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/ByteBufferSerializer.html#toBinary(o:AnyRef,buf:java.nio.ByteBuffer):Unit "akka.serialization.ByteBufferSerializer")[`toBinary`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/ByteBufferSerializer.html#toBinary(java.lang.Object,java.nio.ByteBuffer) "akka.serialization.ByteBufferSerializer") and [`fromBinary`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/ByteBufferSerializer.html#fromBinary(buf:java.nio.ByteBuffer,manifest:String):AnyRef "akka.serialization.ByteBufferSerializer")[`fromBinary`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/ByteBufferSerializer.html#fromBinary(java.nio.ByteBuffer,java.lang.String) "akka.serialization.ByteBufferSerializer") methods also implement the array based [`toBinary`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html#toBinary(o:AnyRef):Array[Byte] "akka.serialization.SerializerWithStringManifest")[`toBinary`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html#toBinary(java.lang.Object) "akka.serialization.SerializerWithStringManifest") and [`fromBinary`](https://doc.akka.io/api/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html#fromBinary(bytes:Array[Byte],manifest:Option[Class[_]]):AnyRef "akka.serialization.SerializerWithStringManifest")[`fromBinary`](https://doc.akka.io/japi/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html#fromBinary(byte[],scala.Option) "akka.serialization.SerializerWithStringManifest") methods. The array based methods will be used when `ByteBuffer` is not used, e.g. in Akka Persistence.
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/scala/docs/actor/ByteBufferSerializerDocSpec.scala#L8-L40 "Go to snippet source")import java.nio.ByteBuffer
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/java/jdocs/actor/ByteBufferSerializerDocTest.java#L8-L57 "Go to snippet source")import akka.serialization.ByteBufferSerializer;
import akka.serialization.SerializerWithStringManifest;
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/scala/docs/routing/RouterDocSpec.scala#L219-L225 "Go to snippet source")akka.actor.deployment {
  /parent/remotePool {
    router = round-robin-pool
    nr-of-instances = 10
    target.nodes = ["tcp://[[email protected]](/cdn-cgi/l/email-protection):2552", "akka://[[email protected]](/cdn-cgi/l/email-protection):2552"]
  }
}
```
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/scala/docs/routing/RouterDocSpec.scala#L241-L249 "Go to snippet source")akka.actor.deployment {
  /parent/remoteGroup2 {
...
The main incompatible change from the previous implementation that the protocol field of the string representation of an [`ActorRef`](https://doc.akka.io/api/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.6/akka/actor/ActorRef.html "akka.actor.ActorRef") is always *akka* instead of the previously used *akka.tcp* or *akka.ssl.tcp*. Configuration properties are also different.
...
See `inbound-lanes` and `outbound-lanes` in the [reference configuration](general/configuration-reference.html#config-akka-remote-artery) for default values.
...
The large messages channel can still not be used for extremely large messages, a few MB per message at most. An alternative is to use the [Reliable delivery](typed/reliable-delivery.html) that has support for automatically [splitting up large messages](typed/reliable-delivery.html#chunk-large-messages) and assemble them again on the receiving side.

### External, shared Aeron media driver

The Aeron transport is running in a so called [media driver](https://github.com/real-logic/Aeron/wiki/Media-Driver-Operation). By default, Akka starts the media driver embedded in the same JVM process as application. This is convenient and simplifies operational concerns by only having one process to start and monitor.
...
You find those jar files on [Maven Central](https://search.maven.org/), or you can create a package with your preferred build tool.

You can pass [Aeron properties](https://github.com/real-logic/Aeron/wiki/Configuration-Options) as command line *\-D* system properties:
...
There are lots of configuration properties that are related to remoting in Akka. We refer to the [reference configuration](general/configuration-reference.html#config-akka-remote-artery) for more information.
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L89-L90 "Go to snippet source")ConfigFactory.parseString("akka.remote.artery.canonical.hostname=\"1.2.3.4\"")
...
You can look at the [Cluster with docker\-compse example project](https://developer.lightbend.com/start/?group=akka&project=akka-sample-cluster-docker-compose-java) [Cluster with docker\-compose example project](https://developer.lightbend.com/start/?group=akka&project=akka-sample-cluster-docker-compose-scala) to see what this looks like in practice.
...
When running on JDK 11 Artery specific flight recording is available through the [Java Flight Recorder (JFR)](https://openjdk.java.net/jeps/328). The flight recorder is automatically enabled by detecting JDK 11 but can be disabled if needed by setting `akka.java-flight-recorder.enabled = false`.
...
```text
val selection =
  context.actorSelection("akka://[email protected]:25520/user/actorName")
```
...
### Example 13: Configuring SSL/TLS for Akka Remoting
...
### Example 14: Configuring SSL/TLS for Akka Remoting
...
### Example 15: Untrusted Mode

```text
akka.remote.artery.untrusted-mode = on
```

### Example 16: Untrusted Mode

```text
akka.remote.artery.trusted-selection-paths = ["/user/receptionist", "/user/namingService"]
```

### Example 17: Failure Detector

```text
phi = -log10(1 - F(timeSinceLastHeartbeat))
```

### Example 18: ByteBuffer based serialization
...
### Example 19: ByteBuffer based serialization
...
### Example 20: ByteBuffer based serialization
...
### Example 21: ByteBuffer based serialization
...
### Example 22: Routers with Remote Destinations
...
akka.actor.deployment {
  /parent/remotePool {
    router = round-robin-pool
    nr-of-instances = 10
    target.nodes = ["tcp://[email protected]:2552", "akka://[email protected]:2552"]
  }
}
```

### Example 23: Routers with Remote Destinations
...
### Example 24: Dedicated subchannel for large messages
...
### Example 25: Dedicated subchannel for large messages
...
### Example 26: Dedicated subchannel for large messages
...
### Example 27: External, shared Aeron media driver

```text
java io.aeron.driver.MediaDriver
```

### Example 28: External, shared Aeron media driver

```text
Agrona-0.5.4.jar:aeron-driver-1.0.1.jar:aeron-client-1.0.1.jar
```

### Example 29: External, shared Aeron media driver

```text
-Daeron.dir=/dev/shm/aeron
```

### Example 30: External, shared Aeron media driver

```text
java io.aeron.driver.MediaDriver config/aeron.properties
```

### Example 31: External, shared Aeron media driver
...
### Example 32: External, shared Aeron media driver
...
### Example 33: Fine-tuning CPU usage latency tradeoff
...
### Example 34: Remote Configuration
...
### Example 35: Akka behind NAT or in a Docker container
...
### Example 36: Running in Docker/Kubernetes
...
- https://doc.akka.io/api/akka-core/2.6/akka/actor/PossiblyHarmful.html
- https://doc.akka.io/api/akka-core/2.6/akka/actor/Terminated.html
- https://doc.akka.io/api/akka-core/2.6/akka/serialization/ByteBufferSerializer.html
- https://doc.akka.io/api/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html
...
- https://doc.akka.io/japi/akka-core/2.6/akka/actor/Kill.html
- https://doc.akka.io/japi/akka-core/2.6/akka/actor/PoisonPill.html
- https://doc.akka.io/japi/akka-core/2.6/akka/actor/PossiblyHarmful.html
- https://doc.akka.io/japi/akka-core/2.6/akka/actor/Terminated.html
- https://doc.akka.io/japi/akka-core/2.6/akka/serialization/ByteBufferSerializer.html
- https://doc.akka.io/japi/akka-core/2.6/akka/serialization/SerializerWithStringManifest.html
...
- https://doc.akka.io/libraries/akka-core/2.6/stream/stream-io.html
...
---
*Source: [https://doc.akka.io/libraries/akka-core/2.6/remoting-artery.html](https://doc.akka.io/libraries/akka-core/2.6/remoting-artery.html)*