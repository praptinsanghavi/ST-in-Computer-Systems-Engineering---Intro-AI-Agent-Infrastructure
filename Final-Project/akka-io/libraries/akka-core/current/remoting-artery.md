...
```
dependencies {
  implementation "io.aeron:aeron-driver:1.45.2"
  implementation "io.aeron:aeron-client:1.45.2"
}
```
Java 17
...
Remoting enables Actor systems on different hosts or JVMs to communicate with each other. By enabling remoting the system will start listening on a provided network address and also gains the ability to connect to other systems through the network. From the application’s perspective there is no API difference between local or remote systems, [`ActorRef`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef") instances that point to remote systems look exactly the same as local ones: they can be sent messages to, watched, etc. Every `ActorRef` contains hostname and port information and can be passed around even on the network. This means that on a network every `ActorRef` is a unique identifier of an actor on that network.
...
In order to communicate with an actor, it is necessary to have its [`ActorRef`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef"). In the local case it is usually the creator of the actor (the caller of `actorOf()`) is who gets the `ActorRef` for an actor that it can then send to other actors. In other words:

- An Actor can get a remote Actor’s reference by receiving a message from it (as it’s available as [`sender()`](https://doc.akka.io/api/akka-core/2.10/akka/actor/Actor.html#sender():akka.actor.ActorRef "akka.actor.Actor")[`getSender()`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/AbstractActor.html#getSender() "akka.actor.AbstractActor") then), or inside of a remote message (e.g. *PleaseReply(message: String, remoteActorRef: ActorRef)*)

Alternatively, an actor can look up another located at a known path using [`ActorSelection`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection"). These methods are available even in remoting enabled systems:

- Remote Lookup : used to look up an actor on a remote node with [`actorSelection(path)`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRefFactory.html#actorSelection(path:String):akka.actor.ActorSelection "akka.actor.ActorRefFactory")[`actorSelection(path)`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRefFactory.html#actorSelection(java.lang.String) "akka.actor.ActorRefFactory")
- Remote Creation : used to create an actor on a remote node with [`actorOf(Props(...), actorName)`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRefFactory.html#actorOf(props:akka.actor.Props,name:String):akka.actor.ActorRef "akka.actor.ActorRefFactory")[`actorOf(Props(...), actorName)`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRefFactory.html#actorOf(akka.actor.Props,java.lang.String) "akka.actor.ActorRefFactory")

In the next sections the two alternatives are described in detail.

### Looking up Remote Actors

[`actorSelection(path)`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRefFactory.html#actorSelection(path:String):akka.actor.ActorSelection "akka.actor.ActorRefFactory")[`actorSelection(path)`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRefFactory.html#actorSelection(java.lang.String) "akka.actor.ActorRefFactory") will obtain an [`ActorSelection`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection") to an Actor on a remote node, e.g.:
...
To acquire an [`ActorRef`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef") for an [`ActorSelection`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection")[`ActorSelection`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorSelection.html "akka.actor.ActorSelection") you need to send a message to the selection and use the [`sender()`](https://doc.akka.io/api/akka-core/2.10/akka/actor/Actor.html#sender():akka.actor.ActorRef "akka.actor.Actor")[`getSender()`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/AbstractActor.html#getSender() "akka.actor.AbstractActor") reference of the reply from the actor. There is a built\-in [`Identify`](https://doc.akka.io/api/akka-core/2.10/akka/actor/Identify.html "akka.actor.Identify")[`Identify`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/Identify.html "akka.actor.Identify") message that all Actors will understand and automatically reply to with a [`ActorIdentity`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorIdentity.html "akka.actor.ActorIdentity")[`ActorIdentity`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorIdentity.html "akka.actor.ActorIdentity") message containing the `ActorRef`. This can also be done with the [`resolveOne`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorSelection.html#resolveOne(timeout:scala.concurrent.duration.FiniteDuration):scala.concurrent.Future[akka.actor.ActorRef] "akka.actor.ActorSelection")[`resolveOne`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorSelection.html#resolveOne(java.time.Duration) "akka.actor.ActorSelection") method of the `ActorSelection`, which returns a [`Future`](http://www.scala-lang.org/api/2.13.17/scala/concurrent/Future.html "scala.concurrent.Future")[`CompletionStage`](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/concurrent/CompletionStage.html "java.util.concurrent.CompletionStage") of the matching `ActorRef`.
...
Under the hood remote death watch uses heartbeat messages and a failure detector to generate [`Terminated`](https://doc.akka.io/api/akka-core/2.10/akka/actor/Terminated.html "akka.actor.Terminated")[`Terminated`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/Terminated.html "akka.actor.Terminated") message from network failures and JVM crashes, in addition to graceful termination of watched actor.
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-actor/src/main/scala/akka/serialization/Serializer.scala#L214-L228 "Go to snippet source")trait ByteBufferSerializer {
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/actor/ByteBufferSerializerDocTest.java#L63-L72 "Go to snippet source")interface ByteBufferSerializer {
  /** Serializes the given object into the `ByteBuffer`. */
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/actor/ByteBufferSerializerDocSpec.scala#L8-L40 "Go to snippet source")import java.nio.ByteBuffer
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/actor/ByteBufferSerializerDocTest.java#L8-L57 "Go to snippet source")import akka.serialization.ByteBufferSerializer;
import akka.serialization.SerializerWithStringManifest;
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/routing/RouterDocSpec.scala#L221-L227 "Go to snippet source")akka.actor.deployment {
  /parent/remotePool {
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/routing/RouterDocSpec.scala#L243-L251 "Go to snippet source")akka.actor.deployment {
  /parent/remoteGroup2 {
...
The main incompatible change from the previous implementation is that the protocol field of the string representation of an [`ActorRef`](https://doc.akka.io/api/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka-core/2.10/akka/actor/ActorRef.html "akka.actor.ActorRef") is always *akka* instead of the previously used *akka.tcp* or *akka.ssl.tcp*. Configuration properties are also different.
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L83-L84 "Go to snippet source")ConfigFactory.parseString("akka.remote.artery.canonical.hostname=\"1.2.3.4\"")
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/remoting/RemoteDeploymentDocSpec.scala#L65-L67 "Go to snippet source")  
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L74-L76 "Go to snippet source")  
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/remoting/RemoteDeploymentDocSpec.scala#L12-L13 "Go to snippet source")import akka.actor.{ Address, AddressFromURIString, Deploy, Props }
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L16-L22 "Go to snippet source")import akka.actor.ActorRef;
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/remoting/RemoteDeploymentDocSpec.scala#L57-L58 "Go to snippet source")val one = AddressFromURIString("akka://sys@host:1234")
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L61-L62 "Go to snippet source")Address addr = new Address("akka", "sys", "host", 1234);
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/scala/docs/remoting/RemoteDeploymentDocSpec.scala#L41 "Go to snippet source")val ref = system.actorOf(Props[SampleActor]().withDeploy(Deploy(scope = RemoteScope(address))))
```

Java

```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-docs/src/test/java/jdocs/remoting/RemoteDeploymentDocTest.java#L65-L66 "Go to snippet source")Props props = Props.create(SampleActor.class).withDeploy(new Deploy(new RemoteScope(addr)));
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-remote/src/test/scala/akka/remote/artery/RemoteDeploymentSpec.scala#L82-L92 "Go to snippet source")akka.remote.deployment {
...
### Example 13: Failure Detector

```text
phi = -log10(1 - F(timeSinceLastHeartbeat))
```

### Example 14: ByteBuffer based serialization
...
### Example 15: ByteBuffer based serialization
...
### Example 16: ByteBuffer based serialization
...
### Example 18: Routers with Remote Destinations
...
### Example 19: Routers with Remote Destinations
...
### Example 20: Dedicated subchannel for large messages
...
### Example 21: Dedicated subchannel for large messages
...
### Example 22: Dedicated subchannel for large messages
...
### Example 23: External, shared Aeron media driver

```text
java io.aeron.driver.MediaDriver
```

### Example 24: External, shared Aeron media driver

```text
Agrona-0.5.4.jar:aeron-driver-1.0.1.jar:aeron-client-1.0.1.jar
```

### Example 25: External, shared Aeron media driver
...
### Example 29: Fine-tuning CPU usage latency tradeoff
...
### Example 32: Running in Docker/Kubernetes
...
### Example 33: Creating Actors Remotely
...
### Example 34: Creating Actors Remotely
...
### Example 35: Creating Actors Remotely
...
### Example 36: Programmatic Remote Deployment
...
### Example 37: Programmatic Remote Deployment
...
### Example 38: Programmatic Remote Deployment
...
### Example 42: Remote deployment allow list
...
- https://doc.akka.io/japi/akka-core/2.10/akka/serialization/ByteBufferSerializer.html
...
- https://doc.akka.io/libraries/akka-core/current/index-cluster.html
- https://doc.akka.io/libraries/akka-core/current/multi-node-testing.html
...
---
*Source: [https://doc.akka.io/libraries/akka-core/current/remoting-artery.html](https://doc.akka.io/libraries/akka-core/current/remoting-artery.html)*