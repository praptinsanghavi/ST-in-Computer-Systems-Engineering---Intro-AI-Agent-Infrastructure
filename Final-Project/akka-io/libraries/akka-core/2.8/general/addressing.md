...
scraped_at: '2026-04-06T14:23:59Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/docs/akka/current/general/addressing.html
...
An actor reference is a subtype of [`ActorRef`](https://doc.akka.io/api/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef"), whose foremost purpose is to support sending messages to the actor it represents. Each actor has access to its canonical (local) reference through the [`ActorContext.self`](https://doc.akka.io/japi/akka/2.8/akka/actor/typed/javadsl/ActorContext.html#getSelf() "akka.actor.typed.javadsl.ActorContext")[`ActorContext.self`](https://doc.akka.io/api/akka/2.8/akka/actor/typed/scaladsl/ActorContext.html#self:akka.actor.typed.ActorRef[T] "akka.actor.typed.scaladsl.ActorContext") field; this reference can be included in messages to other actors to get replies back.
...
- There are several special types of actor references which behave like local actor references for all practical purposes:
	- `PromiseActorRef` is the special representation of a [`Promise`](http://www.scala-lang.org/api/2.13.11/scala/concurrent/Promise.html "scala.concurrent.Promise") for the purpose of being completed by the response from an actor. `akka.pattern.ask` creates this actor reference.
...
- And then there are some one\-off internal implementations which you should never really see:
	...
	- The first logging service started before actually firing up actor creation facilities is a fake actor reference which accepts log events and prints them directly to standard output; it is [`Logging.StandardOutLogger`](https://doc.akka.io/api/akka/2.8/akka/event/Logging$$StandardOutLogger.html "akka.event.Logging.StandardOutLogger")[`Logging.StandardOutLogger`](https://doc.akka.io/japi/akka/2.8/akka/event/Logging.StandardOutLogger.html "akka.event.Logging.StandardOutLogger").
...
Equality of [`ActorRef`](https://doc.akka.io/api/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef") match the intention that an [`ActorRef`](https://doc.akka.io/api/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef") corresponds to the target actor incarnation. Two actor references are compared equal when they have the same path and point to the same actor incarnation. A reference pointing to a terminated actor does not compare equal to a reference pointing to another (re\-created) actor with the same path. Note that a restart of an actor caused by a failure still means that it is the same actor incarnation, i.e. a restart is not visible for the consumer of the [`ActorRef`](https://doc.akka.io/api/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef")[`ActorRef`](https://doc.akka.io/japi/akka/2.8/akka/actor/typed/ActorRef.html "akka.actor.typed.ActorRef").

If you need to keep track of actor references in a collection and do not care about the exact actor incarnation you can use the [`ActorPath`](https://doc.akka.io/api/akka/2.8/akka/actor/ActorPath.html "akka.actor.ActorPath")[`ActorPath`](https://doc.akka.io/japi/akka/2.8/akka/actor/ActorPath.html "akka.actor.ActorPath") as key, because the identifier of the target actor is not taken into account when comparing actor paths.
...
- `"/user"` is the guardian actor for all user\-created top\-level actors; actors created using [`ActorSystem.actorOf`](https://doc.akka.io/api/akka/2.8/akka/actor/ActorRefFactory.html#actorOf(props:akka.actor.Props):akka.actor.ActorRef "akka.actor.ActorRefFactory")[`ActorSystem.actorOf`](https://doc.akka.io/japi/akka/2.8/akka/actor/ActorRefFactory.html#actorOf(akka.actor.Props) "akka.actor.ActorRefFactory") are found below this one.
...
- `"/temp"` is the guardian for all short\-lived system\-created actors, e.g. those which are used in the implementation of [`ActorRef.ask`](https://doc.akka.io/api/akka/2.8/akka/pattern/AskableActorRef.html#ask(message:Any)(implicittimeout:akka.util.Timeout,implicitsender:akka.actor.ActorRef):scala.concurrent.Future[Any] "akka.pattern.AskableActorRef")[`Patterns.ask`](https://doc.akka.io/japi/akka/2.8/akka/pattern/Patterns.html#ask(akka.actor.ActorRef,java.lang.Object,java.time.Duration) "akka.pattern.Patterns").
...
- https://doc.akka.io/api/akka/2.8/akka/event/Logging$$StandardOutLogger.html
...
- https://doc.akka.io/japi/akka/2.8/akka/event/Logging.StandardOutLogger.html
- https://doc.akka.io/japi/akka/2.8/akka/pattern/Patterns.html
- https://doc.akka.io/libraries/akka-core/2.8/general/remoting.html
...
---
*Source: [https://doc.akka.io/libraries/akka-core/2.8/general/addressing.html](https://doc.akka.io/libraries/akka-core/2.8/general/addressing.html)*