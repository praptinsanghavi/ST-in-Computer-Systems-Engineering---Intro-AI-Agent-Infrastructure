...
scraped_at: '2026-04-06T14:25:57Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/docs/akka/current/typed/replicated-eventsourcing.html
title: Replicated Event Sourcing • Akka Documentation
---
...
The [Replicated Event Sourcing video](https://akka.io/blog/news/2020/09/09/replicated-event-sourcing-video) is a good starting point describing the use cases and motivation for when to use Replicated Event Sourcing. The second part, [Replicated Event Sourcing data modelling](https://akka.io/blog/news/2020/10/22/replicated-event-sourcing-data-modelling) guides you to find a suitable model for your use\-case.
...
The same API as regular `EventSourcedBehavior`sA very similar API to the regular `EventSourcedBehavior` is used to define the logic. 
...
The functionality is provided through the Akka Projection gRPC module, see the details about how to use it up in the [Akka Projection gRPC documentation](https://doc.akka.io/docs/akka-projection/current/grpc-replicated-event-sourcing-transport.html)
...
- [`LwwTime`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime")[`LwwTime`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime")
- [`Counter`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/Counter.html "akka.persistence.typed.crdt.Counter")[`Counter`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/Counter.html "akka.persistence.typed.crdt.Counter")
- [`ORSet`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/ORSet.html "akka.persistence.typed.crdt.ORSet")[`ORSet`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/ORSet.html "akka.persistence.typed.crdt.ORSet")
...
An example would be a movies watch list that is represented by the general purpose [`ORSet`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/ORSet.html "akka.persistence.typed.crdt.ORSet")[`ORSet`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/ORSet.html "akka.persistence.typed.crdt.ORSet") CRDT. `ORSet` is short for Observed Remove Set. Elements can be added and removed any number of times. Concurrent add wins over remove. It is an operation based CRDT where the delta of an operation (add/remove) can be represented as an event.
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/scala/docs/akka/persistence/typed/ReplicatedMovieWatchListExampleSpec.scala#L23-L59 "Go to snippet source")object MovieWatchList {
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/java/jdocs/akka/persistence/typed/ReplicatedMovieExample.java#L24-L101 "Go to snippet source")public final class MovieWatchList
...
There is a small utility class [`LwwTime`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime")[`LwwTime`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime") that can be useful for implementing last writer wins semantics. It contains a timestamp representing current time when the event was persisted and an identifier of the replica that persisted it. When comparing two [`LwwTime`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime")[`LwwTime`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/LwwTime.html "akka.persistence.typed.crdt.LwwTime") the greatest timestamp wins. The replica identifier is used if the two timestamps are equal, and then the one from the `replicaId` sorted first in alphanumeric order wins.

Scala

```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/scala/docs/akka/persistence/typed/ReplicatedBlogExampleSpec.scala#L106-L129 "Go to snippet source")private def eventHandler(
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/java/jdocs/akka/persistence/typed/ReplicatedBlogExample.java#L255-L286 "Go to snippet source")@Override
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/scala/docs/akka/persistence/typed/ReplicatedBlogExampleSpec.scala#L69-L102 "Go to snippet source")private def commandHandler(
    ctx: ActorContext[Command],
    replicationContext: ReplicationContext,
    state: BlogState,
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/java/jdocs/akka/persistence/typed/ReplicatedBlogExample.java#L207-L251 "Go to snippet source")@Override
...
Just like for regular `EventSourcedBehavior`s it is possible to tag events along with persisting them. This is useful for later retrival of events for a given tag. The same [API for tagging provided for EventSourcedBehavior](persistence.html#tagging) can be used for replicated event sourced behaviors as well. Tagging is useful in practice to build queries that lead to other data representations or aggregations of these event streams that can more directly serve user queries – known as building the “read side” in [CQRS](cqrs.html) based applications.

Creating read side projections is possible through [Akka Projections](https://doc.akka.io/docs/akka-projection/current/) or through direct usage of the [events by tag queries](../persistence-query.html#eventsbytag-and-currenteventsbytag). 
...
Determining the replica id of the replicated actor itself and the origin replica id of an event is possible through the [`ReplicationContext`](https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/javadsl/ReplicationContext.html "akka.persistence.typed.javadsl.ReplicationContext")[`ReplicationContext`](https://doc.akka.io/api/akka/2.8/akka/persistence/typed/scaladsl/ReplicationContext.html "akka.persistence.typed.scaladsl.ReplicationContext") when the tagger callback is invoked like this:

Scala

```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/scala/akka/persistence/typed/ReplicatedEventSourcingTaggingSpec.scala#L46-L71 "Go to snippet source")ReplicatedEventSourcing.commonJournalConfig(
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-persistence-typed-tests/src/test/java/jdocs/akka/persistence/typed/ReplicatedStringSet.java#L71-L83 "Go to snippet source")@Override
...
For a journal plugin to support replication it needs to store and read metadata for each event if it is defined in the `metadata` field. To attach the metadata after writing it, `PersistentRepr.withMetadata` is used. The [`JournalSpec`](https://doc.akka.io/api/akka/2.8/akka/persistence/journal/JournalSpec.html "akka.persistence.journal.JournalSpec")[`JournalSpec`](https://doc.akka.io/japi/akka/2.8/akka/persistence/journal/JournalSpec.html "akka.persistence.journal.JournalSpec") in the Persistence TCK provides a capability flag `supportsMetadata` to toggle verification that metadata is handled correctly.

For a snapshot plugin to support replication it needs to store and read metadata for the snapshot if it is defined in the `metadata` field. To attach the metadata when reading the snapshot the `akka.persistence.SnapshotMetadata.apply` factory overload taking a `metadata` parameter is used. The [`SnapshotStoreSpec`](https://doc.akka.io/api/akka/2.8/akka/persistence/snapshot/SnapshotStoreSpec.html "akka.persistence.snapshot.SnapshotStoreSpec")[`SnapshotStoreSpec`](https://doc.akka.io/japi/akka/2.8/akka/persistence/snapshot/SnapshotStoreSpec.html "akka.persistence.snapshot.SnapshotStoreSpec") in the Persistence TCK provides a capability flag `supportsMetadata` to toggle verification that metadata is handled correctly.
...
### Example 9: Tagging events and running projections
...
### Example 10: Tagging events and running projections
...
- https://doc.akka.io/api/akka/2.8/akka/persistence/snapshot/SnapshotStoreSpec.html
- https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/Counter.html
- https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/LwwTime.html
- https://doc.akka.io/api/akka/2.8/akka/persistence/typed/crdt/ORSet.html
...
- https://doc.akka.io/docs/akka-projection/current/grpc-replicated-event-sourcing-transport.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/journal/JournalSpec.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/snapshot/SnapshotStoreSpec.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/Counter.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/LwwTime.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/crdt/ORSet.html
- https://doc.akka.io/japi/akka/2.8/akka/persistence/typed/javadsl/ReplicationContext.html
...
- https://doc.akka.io/libraries/akka-core/2.8/typed/cqrs.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/distributed-data.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/persistence.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing-auction.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing-db-transport.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing-examples.html
- https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing.html

---
*Source: [https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing.html](https://doc.akka.io/libraries/akka-core/2.8/typed/replicated-eventsourcing.html)*