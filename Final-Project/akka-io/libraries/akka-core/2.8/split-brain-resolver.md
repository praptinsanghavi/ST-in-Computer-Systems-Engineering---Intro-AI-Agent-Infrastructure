...
scraped_at: '2026-04-06T14:28:22Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/docs/akka/current/split-brain-resolver.html
title: Split Brain Resolver • Akka Documentation
---
...
When operating an Akka cluster you must consider how to handle [network partitions](https://en.wikipedia.org/wiki/Network_partition) (a.k.a. split brain scenarios) and machine crashes (including JVM and hardware failures). This is crucial for correct behavior if you use [Cluster Singleton](typed/cluster-singleton.html) or [Cluster Sharding](typed/cluster-sharding.html), especially together with Akka Persistence.

The [Split Brain Resolver video](https://akka.io/blog/news/2020/06/08/akka-split-brain-resolver-video) is a good starting point for learning why it is important to use a correct downing provider and how the Split Brain Resolver works.
...
To use Akka Split Brain Resolver is part of `akka-cluster` and you probably already have that dependency included. Otherwise, add the following dependency in your project:
...
| --- | --- |
| Artifact | com.typesafe.akka akka\-cluster 2\.8\.9 [Snapshots are available](project/links.html#snapshots-repository) |
...
| Readiness level | [Supported](https://developer.lightbend.com/docs/introduction/getting-help/support-terminology.html#supported), [Lightbend Subscription](https://www.lightbend.com/lightbend-subscription) provides support Since 2\.2\.0, 2013\-07\-09 |
| Home page | <https://akka.io/> |
| API documentation | [API (Scaladoc)](https://doc.akka.io/api/akka/2.8.9/akka/cluster/index.html) [API (Javadoc)](https://doc.akka.io/japi/akka/2.8.9/akka/cluster/package-summary.html) |
...
You need to enable the Split Brain Resolver by configuring it as downing provider in the configuration of the `ActorSystem` (`application.conf`):
...
By default the [Keep Majority](split-brain-resolver.html#keep-majority) strategy will be used because it works well for most systems. However, it’s worth considering the other available strategies and pick a strategy that fits the characteristics of your system. For example, in a Kubernetes environment the [Lease](split-brain-resolver.html#lease) strategy can be a good choice.
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-cluster/src/main/resources/reference.conf#L390-L425 "Go to snippet source")  
...
That is handled by [Coordinated Shutdown](coordinated-shutdown.html) but to exit the JVM it’s recommended that you enable:
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-cluster/src/main/resources/reference.conf#L456-L459 "Go to snippet source")akka.cluster.split-brain-resolver.keep-majority {
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-cluster/src/main/resources/reference.conf#L441-L447 "Go to snippet source")akka.cluster.split-brain-resolver.static-quorum {
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-cluster/src/main/resources/reference.conf#L473-L480 "Go to snippet source")akka.cluster.split-brain-resolver.keep-oldest {
...
See also [Down all when unstable](split-brain-resolver.html#down-all-when-unstable) and [indirectly connected nodes](split-brain-resolver.html#indirectly-connected-nodes).

### Lease

The strategy named `lease-majority` is using a distributed lease (lock) to decide what nodes that are allowed to survive. Only one SBR instance can acquire the lease and make the decision to remain up. The other side will not be able to aquire the lease and will therefore down itself.
...
There is currently one supported implementation of the lease which is backed by a [Custom Resource Definition (CRD)](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) in Kubernetes. It is described in the [Kubernetes Lease](https://doc.akka.io/docs/akka-management/current/kubernetes-lease.html) documentation.
...
Similar to other strategies it is important that decisions are not deferred for too long because the nodes that couldn’t acquire the lease must decide to down themselves, see [Down all when unstable](split-brain-resolver.html#down-all-when-unstable).
...
```
[source](https://github.com/akka/akka/tree/v2.8.9/akka-cluster/src/main/resources/reference.conf#L488-L504 "Go to snippet source")akka.cluster.split-brain-resolver.lease-majority {
...
```
Warning
It is recommended to keep `down-all-when-unstable` enabled and not set it to a longer duration than `stable-after` (`down-removal-margin`) because that can result in delayed decisions on the side that should have been downed, e.g. in the case of a clean network partition followed by continued instability on the side that should be downed. That could result in that members are removed from one side but are still running on the other side.

## Multiple data centers

Akka Cluster has [support for multiple data centers](cluster-dc.html), where the cluster membership is managed by each data center separately and independently of network partitions across different data centers. The Split Brain Resolver is embracing that strategy and will not count nodes or down nodes in another data center.

When there is a network partition across data centers the typical solution is to wait the partition out until it heals, i.e. do nothing. Other decisions should be performed by an external monitoring tool or human operator.
...
In total, you can expect the failover time of a singleton or sharded instance to be around 45 seconds with default configuration. The default configuration is sized for a cluster of 100 nodes. If you have around 10 nodes you can reduce the `stable-after` to around 10 seconds, resulting in an expected failover time of around 25 seconds.
...
### Example 7: Enable the Split Brain Resolver

```text
akka.cluster.downing-provider-class = "akka.cluster.sbr.SplitBrainResolverProvider"
```

### Example 8: Stable after
...
### Example 9: Stable after

```text
akka.coordinated-shutdown.exit-jvm = on
```

### Example 10: Keep Majority

```text
akka.cluster.split-brain-resolver.active-strategy=keep-majority
```

### Example 11: Keep Majority
...
### Example 12: Static Quorum

```text
akka.cluster.split-brain-resolver.active-strategy=static-quorum
```

### Example 13: Static Quorum
...
### Example 14: Keep Oldest

```text
akka.cluster.split-brain-resolver.active-strategy=keep-oldest
```

### Example 15: Keep Oldest
...
### Example 18: Down all when unstable
...
- https://doc.akka.io/api/akka/2.8.9/akka/cluster/index.html
- https://doc.akka.io/docs/akka-management/current/kubernetes-lease.html
- https://doc.akka.io/japi/akka/2.8.9/akka/cluster/package-summary.html
- https://doc.akka.io/libraries/akka-core/2.8/cluster-dc.html
- https://doc.akka.io/libraries/akka-core/2.8/coordinated-shutdown.html
- https://doc.akka.io/libraries/akka-core/2.8/coordination.html
...
- https://doc.akka.io/libraries/akka-core/2.8/typed/cluster-singleton.html

---
*Source: [https://doc.akka.io/libraries/akka-core/2.8/split-brain-resolver.html](https://doc.akka.io/libraries/akka-core/2.8/split-brain-resolver.html)*