...
scraped_at: '2026-04-06T13:43:22Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-management/current/kubernetes-deployment/forming-a-cluster.html
title: Forming an Akka Cluster • Akka Management
---

# Forming an Akka Cluster • Akka Management
...
Services that use Akka Cluster have additional requirements over stateless applications. To form a cluster, each pod needs to know which other pods have been deployed as part of that service, so that they can connect to each other. Akka provides a Cluster Bootstrap library that allows Akka applications in Kubernetes to discover this automatically using the Kubernetes API. The process is roughly as follows:
...
- Akka Management Cluster HTTP: This provides HTTP management endpoints as well as a [cluster health check](../cluster-http-management.html#health-checks)
- [Akka Discovery Kubernetes](../discovery/kubernetes.html): This provides a discovery mechanism that queries the Kubernetes API
- [Akka Bootstrap](../bootstrap/index.html): This bootstraps the cluster from nodes discovered via the Kubernetes API
...
Since we are just using the default service account, we need to bind our role to the default service account so that our pod will be able to access the Kubernetes API as a `pod-reader`. In `kubernetes/akka-cluster.yaml`:
...
Akka management HTTP includes [health check routes](../healthchecks.html) that will expose liveness and readiness health checks on `/alive` and `/ready` respectively. 
...
Starting from Kubernetes v1\.22, ReplicaSets are not scaled down with the youngest node first which can cause problems for the Akka cluster. A new Akka extension was developed to address this issue, and you can find the documentation under [Kubernetes Rolling Updates](../rolling-updates.html#kubernetes-rolling-updates) section.

## Code Examples

### Example 1: Bootstrap and Management dependencies
...
### Example 2: Bootstrap and Management dependencies
...
### Example 3: Bootstrap and Management dependencies
...
### Example 5: Akka Cluster Bootstrap
...
---
*Source: [https://doc.akka.io/libraries/akka-management/current/kubernetes-deployment/forming-a-cluster.html](https://doc.akka.io/libraries/akka-management/current/kubernetes-deployment/forming-a-cluster.html)*