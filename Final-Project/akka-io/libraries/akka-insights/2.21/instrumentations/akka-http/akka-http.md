...
scraped_at: '2026-04-06T01:24:10Z'
section: libraries
site: akka-io
source_url: /instrumentations/akka-http/akka-http.html
...
Akka Insights is capable of capturing data for the following [Akka HTTP](https://doc.akka.io/docs/akka-http/current/introduction.html) related features.
...
```
dependencies {
  implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-akka-http_2.13', version: '2.21.4'
}
```
...
```
dependencies {
  implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-akka-grpc_2.13', version: '2.21.4'
}
```
...
Note
The client metrics can disappear because of the connection pool inactivity. It happens because the Akka HTTP client connection pool is by default configured [to shut down](https://doc.akka.io/docs/akka-http/current/client-side/host-level.html#pool-shutdown) after 30\-second idle time. When the connection pool shuts down, all the associated metrics get destroyed to avoid memory leaks. If after that the same host is requested, another connection pool will be created with a new set of **zeroed** metrics attached to it.

In order to avoid Akka HTTP client metrics reset, the connection pool idle timeout [`akka.http.host-connection-pool.idle-timeout`](https://doc.akka.io/docs/akka-http/current/common/timeouts.html#idle-timeouts) can be increased or even disabled. It can be configured for all or [individual hosts and even by pattern](https://doc.akka.io/docs/akka-http/current/client-side/configuration.html#per-host-overrides).
...
- https://doc.akka.io/docs/akka-http/current/client-side/configuration.html
...
- https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka-projection/akka-projection.html
- https://doc.akka.io/libraries/akka-insights/2.21/setup/setup.html

---
*Source: [https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka-http/akka-http.html](https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka-http/akka-http.html)*