...
scraped_at: '2026-04-06T01:24:11Z'
section: libraries
site: akka-io
source_url: /instrumentations/akka/akka.html
...
```
dependencies {
  implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-akka_2.13', version: '2.21.4'
}
```
...
Note
Router metrics are only available for [router actors](https://doc.akka.io/docs/akka/current/routing.html#a-router-actor), i.e. not available when routers are [used directly](https://doc.akka.io/docs/akka/current/routing.html#a-simple-router).
...
- **Phi accrual value** (gauge) — statistics for the [Phi accrual failure detector](https://doc.akka.io/docs/akka/current/remoting.html#failure-detector). A Phi value represents the connection between two nodes; self and remote. A self node can have a connection to any number of remote nodes and each connection will have its own Phi value. *Note that internally in Akka the Phi accrual value can become Double.Infinity. If this happens Cinnamon will convert this value to 1024\*1024\. The reason for this is that most visualizers cannot handle infinity. If you therefore see the value 1048576 (1024\*1024\) this means that the Phi value has reached infinity.*
...
- **Current cluster state event** — a one time event, per cluster node, containing information about the [state of the cluster](https://doc.akka.io/docs/akka/current/cluster-usage.html#subscribe-to-cluster-events).
...
Reachable and unreachable nodes are based on observations from the [failure detector](https://doc.akka.io/docs/akka/current/common/cluster.html#failure-detector) and is orthogonal to the cluster [member states](https://doc.akka.io/docs/akka/current/common/cluster.html#member-states). In other words, a member in, for example, status `Up` can be either `reachable` or `unreachable`.

## Split Brain Resolver events

Running [Split Brain Resolver](https://doc.akka.io/docs/akka/current/split-brain-resolver.html) in your cluster will ensure better resilience. If you have SBR running, Akka Insights will automatically keep track of any activity therein. If there is a split in your cluster, events will be generated.
...
<dependency>
  <groupId>com.lightbend.cinnamon</groupId>
  <artifactId>cinnamon-akka-cluster_2.13</artifactId>
  <version>2.21.4</version>
</dependency>
```
...
// Use Akka Cluster instrumentation
dependencies {
    implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-akka-cluster_2.13', version: '2.21.4'
}
```
...
Akka [Classic](https://doc.akka.io/docs/akka/current/actors.html#ask-send-and-receive-future) and [Typed](https://doc.akka.io/docs/akka/current/typed/interaction-patterns.html#request-response-with-ask-from-outside-an-actor) provide two ways to interact with actors:
...
### Example 9: Split Brain Resolver events
...
### Example 10: Split Brain Resolver events
...
### Example 11: Split Brain Resolver events
...
- https://doc.akka.io/docs/akka/current/common/cluster.html
...
- https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka/dispatchers.html
- https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/metrics-and-events/metrics-and-events.html
- https://doc.akka.io/libraries/akka-insights/2.21/setup/setup.html

---
*Source: [https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka/akka.html](https://doc.akka.io/libraries/akka-insights/2.21/instrumentations/akka/akka.html)*