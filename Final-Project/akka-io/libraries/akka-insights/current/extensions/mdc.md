...
scraped_at: '2026-04-05T19:28:16Z'
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-insights/current/home.html/extensions/mdc.html
...
```
dependencies {
  implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-slf4j-mdc_2.13', version: '2.22.2'
}
```
...
Note
You do not need to use the [DiagnosticLoggingAdapter](https://doc.akka.io/libraries/akka-core/current/logging.html#mdc-values-defined-by-the-application) or other MDC utilities in Akka when using the Cinnamon MDC instrumentation. The current MDC values are automatically transferred to the logging actors. There is also no need for special MDC cleanup; Cinnamon MDC will swap the MDC in and out of context as messages or callbacks are processed.
...
Cinnamon supports automatically inserting random correlation IDs. For example: on HTTP server requests, if there is no correlation ID already in context, Cinnamon can add a random UUID to the MDC. 

#### Generating UUIDs

Warning
By default, Cinnamon uses `java.util.UUID` which has the potential to impact your application performance. 

There are two options to improve this.

The first option is to run your JVM with `-Djava.security.egd=file:/dev/./urandom` to get hold of a more performant random number generator that avoids blocking when the entropy pool is depleted.

The second option is use a different implementation to generate UUID by adding the following dependency to your project:

sbt

```
libraryDependencies += Cinnamon.library.cinnamonFasterXMLUUID
```
...
<dependency>
  <groupId>com.lightbend.cinnamon</groupId>
  <artifactId>cinnamon-fasterxml-uuid</artifactId>
  <version>2.22.2</version>
</dependency>
```
...
```
dependencies {
  implementation group: 'com.lightbend.cinnamon', name: 'cinnamon-fasterxml-uuid', version: '2.22.2'
}
```

Now, after you also have to configure the new UUID generator like this:

```
cinnamon.uuid.generator = fasterxml
```

The injected correlation ID will then be propagated with the MDC and can be included in all log messages associated with the initial request, including other HTTP requests, actors, streams, or futures. The correlation ID will also be automatically included for propagation if an [MDC filter](mdc.html#mdc-filter) is configured.
...
### Example 19: Generating UUIDs

```sbt
libraryDependencies += Cinnamon.library.cinnamonFasterXMLUUID
```

### Example 20: Generating UUIDs
...
### Example 22: Generating UUIDs

```scala
cinnamon.uuid.generator = fasterxml
```

### Example 23: Generating UUIDs
...
### Example 26: Normalizing correlation ID key names
...
### Example 28: OpenTracing trace IDs
...
### Example 29: OpenTracing trace IDs
...
### Example 30: OpenTracing trace IDs
...
### Example 31: OpenTracing trace IDs
...
### Example 32: Trace ID normalization
...
- https://doc.akka.io/libraries/akka-insights/current/extensions/opentracing/opentracing.html
...
---
*Source: [https://doc.akka.io/libraries/akka-insights/current/extensions/mdc.html](https://doc.akka.io/libraries/akka-insights/current/extensions/mdc.html)*