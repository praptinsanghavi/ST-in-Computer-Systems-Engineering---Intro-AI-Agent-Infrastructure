...
section: libraries
site: akka-io
source_url: https://doc.akka.io/libraries/akka-persistence-cassandra/current/migrations.html
title: Migration • Akka Persistence Cassandra
---

# Migration • Akka Persistence Cassandra
...
See [reference.conf](https://github.com/akka/akka-persistence-cassandra/blob/main/core/src/main/resources/reference.conf) for details and update your `application.conf` accordingly.
...
Create the `all_persistence_ids` table if you are not using `tables-autocreate=on`, which is not recommended for production. See [table definition](journal.html#schema).
...
```
[source](https://github.com/akka/akka-persistence-cassandra/tree/v1.3.4/core/src/test/scala/doc/reconciler/AllPersistenceIdsMigrationCompileOnly.scala#L8-L13 "Go to snippet source")import scala.util.Failure
...
If you migrate directly to 1\.0\.0 you must first run the migration of the [All persistenceIds query](migrations.html#all-persistenceids-query).
...
You do not need to worry if a small number of events are missed by `migrateToTagViews` as they will be fixed during your `PersistentActor` recovery. However do not rely on this for full migration as only active `PersistentActor`s will be recovered and it will mean the start up time for your `PersistentActor`s will be very long. Moreover, if your application consumes events through `eventsByTag` query, the missed events will only show up on the query side once you instantiate the respective `PersistentActor`. 
...
When trying to create the materialized view (tables\-autocreate\=on) with the meta columns before corresponding columns have been added the messages table an exception “Undefined column name meta\_ser\_id” is raised, because Cassandra validates the [“CREATE MATERIALIZED VIEW IF NOT EXISTS”](https://docs.datastax.com/en/cql/3.3/cql/cql_reference/cqlCreateMaterializedView.html#cqlCreateMaterializedView__if-not-exists) even though the view already exists and will not be created. To work around that issue you can disable the meta columns in the materialized view by setting `meta-in-events-by-tag-view=off`.
...
### Example 1: Changed configuration structure
...
### Example 2: Changed configuration structure
...
### Example 3: Drop static column

```text
alter table akka.messages drop used;
```

### Example 4: All persistenceIds query
...
### Example 5: Migrations to 0.101 and later in 0.x series

```text
cassandra-journal.write-static-column-compat = off
```

### Example 6: Migrations to 0.101 and later in 0.x series

```text
alter table akka.messages drop used;
```

### Example 7: Migrations to 0.80 and later in 0.x series
...
### Example 8: Migrations to 0.80 and later in 0.x series

```cql
DROP MATERIALIZED VIEW akka.eventsbytag;
```

### Example 9: Migrations to 0.80 and later in 0.x series
...
### Example 10: Migrations from 0.54 to 0.59
...
### Example 11: Migrations from 0.54 to 0.59

```text
cassandra-journal.meta-in-events-by-tag-view = off
```

### Example 12: Migrations from 0.51 to 0.52

```scala
"com.typesafe.akka" %% "akka-persistence-cassandra-launcher" % "0.52"
```

### Example 13: Migrations from 0.51 to 0.52
...
### Example 14: Migrations from 0.9 to 0.10
...
- https://doc.akka.io/
- https://doc.akka.io/libraries/akka-core/2.5/project/migration-guide-2.4.x-2.5.x.html
...
---
*Source: [https://doc.akka.io/libraries/akka-persistence-cassandra/current/migrations.html](https://doc.akka.io/libraries/akka-persistence-cassandra/current/migrations.html)*