---
description: Migrating to Akka 2.6.
knowledge_type: official_documentation
scraped_at: '2026-04-06T14:28:28Z'
section: docs
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/project/migration-guide-2.5.x-2.6.x.html
title: Migration Guide 2.5.x to 2.6.x • Akka Documentation
---

# Migration Guide 2.5.x to 2.6.x • Akka Documentation

> **Summary:** Migrating to Akka 2.6.

## Content

# Migration Guide 2\.5\.x to 2\.6\.x

An overview of the changes in Akka 2\.6 is presented in the [What’s new in Akka 2\.6 video](https://akka.io/blog/news/2019/12/12/akka-26-intro) and the [release announcement](https://akka.io/blog/news/2019/11/06/akka-2.6.0-released).

Akka 2\.6\.x is binary backwards compatible with 2\.5\.x with the ordinary exceptions listed in the [Binary Compatibility Rules](../common/binary-compatibility-rules.html).
...
Rolling updates are possible without shutting down all nodes of the Akka Cluster, but will require configuration adjustments as described in the [Remoting](migration-guide-2.5.x-2.6.x.html#remoting) section of this migration guide. Due to the [changed serialization of the Cluster messages in Akka 2\.6\.2](rolling-update.html#2-6-2-clustermessageserializer-manifests-change) a rolling update from 2\.5\.x must first be made to Akka 2\.6\.2 and then a second rolling update can change to Akka 2\.6\.3 or later.
...
For alternatives see the [documentation about Downing](../typed/cluster.html#downing).
...
- Netty UDP (Classic remoting over UDP)
	- To continue to use UDP configure [Artery UDP](../remoting-artery.html#configuring-ssl-tls-for-akka-remoting) or migrate to Artery TCP.
...
- `UntypedPersistentActorWithAtLeastOnceDelivery`
	- Use [`AbstractPersistentActorWithAtLeastOnceDelivery`](https://doc.akka.io/api/akka-core/2.6/akka/persistence/AbstractPersistentActorWithAtLeastOnceDelivery.html "akka.persistence.AbstractPersistentActorWithAtLeastOnceDelivery")[`AbstractPersistentActorWithAtLeastOnceDelivery`](https://doc.akka.io/japi/akka-core/2.6/akka/persistence/AbstractPersistentActorWithAtLeastOnceDelivery.html "akka.persistence.AbstractPersistentActorWithAtLeastOnceDelivery") instead.
...
[Migration guide to Persistence Typed](../persistence-fsm.html) is in the PersistentFSM documentation.
...
- Latency of calls over a network are several orders of magnitudes longer than latency of local calls, which can be more than surprising if encoded as an innocent looking local method call.
...
Therefore explicit message passing is preferred. It looks different from local method calls (`actorRef ! message``actorRef.tell(message)`) and there is no misconception that sending a message will result in it being processed instantaneously. The goal of location transparency is to unify message passing for both local and remote interactions, versus attempting to make remote interactions look like local method calls.

Warnings about `TypedActor` have been [mentioned in documentation](https://doc.akka.io/libraries/akka-core/2.5/typed-actors.html#when-to-use-typed-actors) for many years.

### Cluster Client

Cluster client has been deprecated as of 2\.6\.0 in favor of [Akka gRPC](https://doc.akka.io/libraries/akka-grpc/current/index.html). It is not advised to build new applications with Cluster client, and existing users [should migrate to Akka gRPC](../cluster-client.html#migration-to-akka-grpc).
...
It is now recommended to use [`ByteString`](https://doc.akka.io/api/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")[`ByteString`](https://doc.akka.io/japi/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")`.emptyByteString()` instead of [`ByteString`](https://doc.akka.io/api/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")[`ByteString`](https://doc.akka.io/japi/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")`.empty()` when using Java because [`ByteString`](https://doc.akka.io/api/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")[`ByteString`](https://doc.akka.io/japi/akka-core/2.6/akka/util/ByteString.html "akka.util.ByteString")`.empty()` is [no longer available as a static method](https://github.com/scala/bug/issues/11509) in the artifacts built for Scala 2\.13\.
...
See documentation of [streaming IO with TLS](../stream/stream-io.html#tls). 
...
[Artery TCP](../remoting-artery.html) is now the default remoting implementation. Classic remoting has been deprecated and will be removed in `2.7.0`.
...
Artery defaults to TCP (see [selected transport](../remoting-artery.html#selecting-a-transport)) which is a good start when migrating from classic remoting.

The protocol part in the Akka `Address`, for example `"akka.tcp://[[email protected]](/cdn-cgi/l/email-protection):2552/user/actorName"` has changed from `akka.tcp` to `akka`. If you have configured or hardcoded any such addresses you have to change them to `"akka://[[email protected]](/cdn-cgi/l/email-protection):25520/user/actorName"`. `akka` is used also when TLS is enabled. One typical place where such address is used is in the `seed-nodes` configuration.
...
The configuration for Artery is different, so you might have to revisit any custom configuration. See the full [reference configuration for Artery](../general/configuration-reference.html#config-akka-remote-artery) and [reference configuration for classic remoting](../general/configuration-reference.html#config-akka-remote).
...
If using SSL then `tcp-tls` needs to be enabled and setup. See [Artery docs for SSL](../remoting-artery.html#configuring-ssl-tls-for-akka-remoting) for how to do this.
...
Classic remoting is deprecated but can be used in 2\.6\.x Rolling update from Classic remoting to Artery is not supported so if you want to update from Akka 2\.5\.x with Classic remoting to Akka 2\.6\.x without a full shut down of the Cluster you have to enable Classic remoting. Later, you can plan for a full shutdown and [migrate from classic remoting to Artery](migration-guide-2.5.x-2.6.x.html#migrating-from-classic-remoting-to-artery) as a separate step.

Explicitly disable Artery by setting property `akka.remote.artery.enabled` to `false`. Further, any configuration under `akka.remote` that is specific to classic remoting needs to be moved to `akka.remote.classic`. To see which configuration options are specific to classic search for them in: [`akka-remote/reference.conf`](../general/configuration-reference.html#config-akka-remote).

If you have a [Lightbend Subscription](https://www.lightbend.com/lightbend-subscription) you can use our [Config Checker](https://doc.akka.io/libraries/akka-enhancements/current/config-checker.html) enhancement to flag any settings that have not been properly migrated.

### Persistent mode for Cluster Sharding

Cluster Sharding coordinator and [Remembering Entities](../cluster-sharding.html#remembering-entities) state could previously be stored in Distributed Data or via Akka Persistence. The Persistence mode has been deprecated in favour of using the Distributed Data mode for the coordinator state. A replacement for the state for Remembered Entities is tracked in [issue 27763](https://github.com/akka/akka/issues/27763).
...
Please see the [rolling update procedure from Java serialization to Jackson](../additional/rolling-updates.html#from-java-serialization-to-jackson).
...
Classic remoting depends on Netty and Artery UDP depends on Aeron. These are now both optional dependencies that need to be explicitly added. See [classic remoting](../remoting.html) or [artery remoting](../remoting-artery.html) for instructions.
...
Watching an actor on a node outside the cluster may have unexpected [consequences](../remoting-artery.html#quarantine), such as quarantining so it has been disabled by default in Akka 2\.6\.x This is the case if either cluster is not used at all (only plain remoting) or when watching an actor outside of the cluster.
...
The deprecated `schedule` method had the same semantics as `scheduleAtFixedRate`, but since that can result in bursts of scheduled tasks or messages after long garbage collection pauses and in the worst case cause undesired load on the system `scheduleWithFixedDelay` is often preferred.
...
For maximum performance, you might want to use a single shared dispatcher for all non\-blocking, asynchronous actors, user actors and Akka internal actors. In that case, you can configure the `akka.actor.internal-dispatcher` with a string value of `akka.actor.default-dispatcher`. This reinstantiates the behavior from previous Akka versions but also removes the isolation between user and Akka internals. So, use at your own risk!
...
For more details about configuring dispatchers, see the [Dispatchers](../dispatchers.html)
...
It is always disabled if [Remembering Entities](../cluster-sharding.html#remembering-entities) is enabled.

#### Cluster Sharding stats

A new field has been added to the response of a `ShardRegion.GetClusterShardingStats` command for any shards per region that may have failed or not responded within the new configurable `akka.cluster.sharding.shard-region-query-timeout`. This is described further in [inspecting sharding state](../cluster-sharding.html#inspecting-cluster-sharding-state).
...
When `ActorSystem.terminate()` is called, [`CoordinatedShutdown`](../coordinated-shutdown.html) will be run in Akka 2\.6\.x, which wasn’t the case in 2\.5\.x. For example, if using Akka Cluster this means that member will attempt to leave the cluster gracefully.
...
Instead, `system.registerOnTermination` or `CoordinatedShutdown` can be used for running such tasks when shutting down.

### IOSources \& FileIO

`FileIO.toPath`, `StreamConverters.fromInputStream`, and `StreamConverters.fromOutputStream` now always fail the materialized value in case of failure. It is no longer required to both check the materialized value and the `Try[Done]` inside the [`IOResult`](https://doc.akka.io/api/akka-core/2.6/akka/stream/IOResult.html "akka.stream.IOResult")[`IOResult`](https://doc.akka.io/japi/akka-core/2.6/akka/stream/IOResult.html "akka.stream.IOResult"). In case of an IO failure the exception will be [`IOOperationIncompleteException`](https://doc.akka.io/api/akka-core/2.6/akka/stream/IOOperationIncompleteException.html "akka.stream.IOOperationIncompleteException")[`IOOperationIncompleteException`](https://doc.akka.io/japi/akka-core/2.6/akka/stream/IOOperationIncompleteException.html "akka.stream.IOOperationIncompleteException") instead of [`AbruptIOTerminationException`](https://doc.akka.io/api/akka-core/2.6/akka/stream/AbruptIOTerminationException.html "akka.stream.AbruptIOTerminationException")[`AbruptIOTerminationException`](https://doc.akka.io/japi/akka-core/2.6/akka/stream/AbruptIOTerminationException.html "akka.stream.AbruptIOTerminationException").
...
The [Lightbend Config Library](https://github.com/lightbend/config) has been updated to load both `reference.conf` and user config files such as `application.conf` before substitution of variables used in the `reference.conf`. This makes it possible to override such variables in `reference.conf` with user configuration.
...
`StreamRefs` was marked as [may change](../common/may-change.html).
...
The path change means that the receptionist information will not be disseminated between 2\.5\.x and 2\.6\.x nodes during a rolling update from 2\.5\.x to 2\.6\.x if you use Akka Typed. See [rolling updates with typed Receptionist](../additional/rolling-updates.html#akka-typed-with-receptionist-or-cluster-receptionist)
...
This means that the receptionist information will not be disseminated between 2\.5\.x and 2\.6\.x nodes during a rolling update from 2\.5\.x to 2\.6\.x if you use Akka Typed. See [rolling updates with typed Cluster Receptionist](../additional/rolling-updates.html#akka-typed-with-receptionist-or-cluster-receptionist)

### Akka Typed API changes

Akka Typed APIs were still marked as [may change](../common/may-change.html) in Akka 2\.5\.x and a few changes were made before finalizing the APIs. Compared to Akka 2\.5\.x the source incompatible changes are:

- `Behaviors.intercept` now takes a factory function for the interceptor.
- `ActorSystem.scheduler` previously gave access to the classic `akka.actor.Scheduler` but now returns a specific `akka.actor.typed.Scheduler`. Additionally `schedule` method has been replaced by `scheduleWithFixedDelay` and `scheduleAtFixedRate`. Actors that need to schedule tasks should prefer `Behaviors.withTimers`.
- `TimerScheduler.startPeriodicTimer`, replaced by `startTimerWithFixedDelay` or `startTimerAtFixedRate`
...
- Java [`EntityRef`](https://doc.akka.io/japi/akka-core/2.6/akka/cluster/sharding/typed/javadsl/EntityRef.html "akka.cluster.sharding.typed.javadsl.EntityRef") ask timeout now takes a `java.time.Duration` rather than a [`Timeout`](https://doc.akka.io/api/akka-core/2.6/akka/util/Timeout.html "akka.util.Timeout")[`Timeout`](https://doc.akka.io/japi/akka-core/2.6/akka/util/Timeout.html "akka.util.Timeout")
- Changed method signature for `EventAdapter.fromJournal` and support for `manifest` in `EventAdapter`.
- Renamed `widen``Behaviors.widen` to `transformMessages``Behaviors.transformMessages`
- `BehaviorInterceptor`, `Behaviors.monitor`, `Behaviors.withMdc` and `transformMessages``Behaviors.transformMessages` takes a `ClassTag` parameter (probably source compatible)`interceptMessageClass` parameter. `interceptMessageType` method in `BehaviorInterceptor` is replaced with this `ClassTag``Class` parameter.
...
- Akka Typed is now using SLF4J as the logging API. `ActorContext.log``ActorContext.getLog` returns an `org.slf4j.Logger`. MDC has been changed to only support `String` values.
- `setLoggerClass` in `ActorContext` has been renamed to `setLoggerName`.
- `GetDataDeleted` and `UpdateDataDeleted` introduced as described in [DataDeleted](migration-guide-2.5.x-2.6.x.html#datadeleted).
...
- `ActorContext` is now a mandatory constructor parameter in `AbstractBehavior`. Create via `Behaviors.setup`. The reason is to encourage right usage and detect mistakes like not creating a new instance (via `setup`) when the behavior is supervised and restarted.
...
Details about the stream materializer can be found in [Actor Materializer Lifecycle](../stream/stream-flows-and-basics.html#actor-materializer-lifecycle)
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-stream-tests/src/test/scala/akka/stream/StreamAttributeDocSpec.scala#L27-L35 "Go to snippet source")val stream: RunnableGraph[Future[Done]] =
  Source(1 to 10)
    .map(_.toString)
...
```
[source](https://github.com/akka/akka/tree/v2.6.21/akka-stream-tests/src/test/java/akka/stream/StreamAttributeDocTest.java#L45-L57 "Go to snippet source")RunnableGraph<CompletionStage<Done>> stream =
    Source.range(1, 10)
        .map(Object::toString)
...
The operators that provide support for lazy and `Future``CompletionStage` stream construction were revised to be more consistent.

The materialized value is now no longer wrapped in an `Option``Optional`, instead the `Future``CompletionStage` is failed with a `akka.stream.NeverMaterializedException` in the cases that would previously lead to `None`an empty `Optional` 

A deferred creation of the stream based on the initial element like how the deprecated `lazyInit` worked can be achieved by combining `future(Flow|Sink)` `completionStage(Flow|Sink)` with `prefixAndTail`. See example in [futureFlow](../stream/operators/Flow/futureFlow.html) [completionStageFlow](../stream/operators/Flow/completionStageFlow.html). 

#### javadsl.Flow

| old | new |
| --- | --- |
| lazyInit | [lazyCompletionStageFlow](../stream/operators/Flow/lazyCompletionStageFlow.html) in combination with `prefixAndTail(1)` |
| lazyInitAsync | [lazyCompletionStageFlow](../stream/operators/Flow/lazyCompletionStageFlow.html) |
|  | [completionStageFlow](../stream/operators/Flow/completionStageFlow.html) |
|  | [lazyFlow](../stream/operators/Flow/lazyFlow.html) |

### javadsl.Sink

| old | new |
| --- | --- |
| lazyInit | [lazyCompletionStageSink](../stream/operators/Sink/lazyCompletionStageSink.html) in combination with `Flow.prefixAndTail(1)` |
| lazyInitAsync | [lazyCompletionStageSink](../stream/operators/Sink/lazyCompletionStageSink.html) |
|  | [completionStageSink](../stream/operators/Sink/completionStageSink.html) |
|  | [lazySink](../stream/operators/Sink/lazySink.html) |

### javadsl.Source

| old | new |
| --- | --- |
| fromFuture | [future](../stream/operators/Source/future.html) |
| fromCompletionStage | [completionStage](../stream/operators/Source/completionStage.html) |
| fromFutureSource | [futureSource](../stream/operators/Source/futureSource.html) |
| fromSourceCompletionStage | [completionStageSource](../stream/operators/Source/completionStageSource.html) |
| lazily | [lazySource](../stream/operators/Source/lazySource.html) |
| lazilyAsync | [lazyCompletionStage](../stream/operators/Source/lazyCompletionStage.html) |
|  | [lazySingle](../stream/operators/Source/lazySingle.html) |
|  | [lazyCompletionStageSource](../stream/operators/Source/lazyCompletionStageSource.html) |

### scaladsl.Flow

| old | new |
| --- | --- |
| lazyInit | [lazyFutureFlow](../stream/operators/Flow/lazyFutureFlow.html) |
| lazyInitAsync | [lazyFutureFlow](../stream/operators/Flow/lazyFutureFlow.html) |
|  | [futureFlow](../stream/operators/Flow/futureFlow.html) |
|  | [lazyFlow](../stream/operators/Flow/lazyFlow.html) |
...
| old | new |
| --- | --- |
| lazyInit | [lazyFutureSink](../stream/operators/Sink/lazyFutureSink.html) in combination with `Flow.prefixAndTail(1)` |
| lazyInitAsync | [lazyFutureSink](../stream/operators/Sink/lazyFutureSink.html) |
|  | [futureSink](../stream/operators/Sink/futureSink.html) |
|  | [lazySink](../stream/operators/Sink/lazySink.html) |
...
| lazily | [lazySource](../stream/operators/Source/lazySource.html) |
| lazilyAsync | [lazyFuture](../stream/operators/Source/lazyFuture.html) |
|  | [lazySingle](../stream/operators/Source/lazySingle.html) |
|  | [lazyFutureSource](../stream/operators/Source/lazyFutureSource.html) |

## Code Examples

### Example 1: Migrating from classic remoting to Artery
...
### Example 3: Java Serialization

```ruby
akka.actor.warn-about-java-serializer-usage = off
```

### Example 4: Remote watch and deployment have been disabled without Cluster use

```text
akka.remote.use-unsafe-remote-features-outside-cluster = on`.
```

### Example 5: Remote watch and deployment have been disabled without Cluster use

```text
akka.remote.warn-unsafe-watch-outside-cluster = off
```

### Example 6: Passivate idle entity

```text
akka.cluster.sharding.passivate-idle-entity-after = off
```

### Example 7: Config for message payload size
...
### Example 8: CoordinatedShutdown is run from ActorSystem.terminate

```text
akka.coordinated-shutdown.run-by-actor-system-terminate = off
```

### Example 9: Cluster failure detection

```text
akka.cluster.monitored-by-nr-of-members = 9
```

### Example 10: Config library resolution change

```ruby
akka.cluster.sharding.distributed-data = ${akka.cluster.distributed-data}
```

### Example 11: Config library resolution change
...
### Example 12: Config library resolution change

```ruby
akka.cluster.distributed-data.gossip-interval = 3s
```

### Example 13: Config library resolution change
...
### Example 14: Materializer settings deprecated
...
### Example 15: Materializer settings deprecated
...
- https://doc.akka.io/api/akka-core/2.6/akka/stream/AbruptIOTerminationException.html
- https://doc.akka.io/api/akka-core/2.6/akka/stream/IOOperationIncompleteException.html
- https://doc.akka.io/api/akka-core/2.6/akka/stream/IOResult.html
...
- https://doc.akka.io/docs/akka/2.6/cluster-client.html
- https://doc.akka.io/docs/akka/2.6/cluster-sharding.html
- https://doc.akka.io/docs/akka/2.6/common/binary-compatibility-rules.html
- https://doc.akka.io/docs/akka/2.6/common/may-change.html
- https://doc.akka.io/docs/akka/2.6/coordinated-shutdown.html
- https://doc.akka.io/docs/akka/2.6/dispatchers.html
- https://doc.akka.io/docs/akka/2.6/general/configuration-reference.html
- https://doc.akka.io/docs/akka/2.6/persistence-fsm.html
- https://doc.akka.io/docs/akka/2.6/project/migration-guide-2.4.x-2.5.x.html
- https://doc.akka.io/docs/akka/2.6/project/migration-guide-2.5.x-2.6.x.html
- https://doc.akka.io/docs/akka/2.6/project/migration-guides.html
- https://doc.akka.io/docs/akka/2.6/project/rolling-update.html
- https://doc.akka.io/docs/akka/2.6/remoting-artery.html
- https://doc.akka.io/docs/akka/2.6/remoting.html
- https://doc.akka.io/docs/akka/2.6/scheduler.html
- https://doc.akka.io/docs/akka/2.6/serialization-jackson.html
- https://doc.akka.io/docs/akka/2.6/serialization.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Flow/completionStageFlow.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Flow/futureFlow.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Flow/lazyCompletionStageFlow.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Flow/lazyFlow.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Flow/lazyFutureFlow.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Sink/completionStageSink.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Sink/futureSink.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Sink/lazyCompletionStageSink.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Sink/lazyFutureSink.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Sink/lazySink.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/completionStage.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/completionStageSource.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/future.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/futureSource.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazyCompletionStage.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazyCompletionStageSource.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazyFuture.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazyFutureSource.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazySingle.html
- https://doc.akka.io/docs/akka/2.6/stream/operators/Source/lazySource.html
- https://doc.akka.io/docs/akka/2.6/stream/stream-flows-and-basics.html
- https://doc.akka.io/docs/akka/2.6/stream/stream-io.html
- https://doc.akka.io/docs/akka/2.6/typed/cluster-sharding.html
- https://doc.akka.io/docs/akka/2.6/typed/cluster-singleton.html
- https://doc.akka.io/docs/akka/2.6/typed/cluster.html

---
*Source: [https://doc.akka.io/docs/akka/2.6/project/migration-guide-2.5.x-2.6.x.html](https://doc.akka.io/docs/akka/2.6/project/migration-guide-2.5.x-2.6.x.html)*