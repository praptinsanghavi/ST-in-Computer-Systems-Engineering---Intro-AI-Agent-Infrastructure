...
scraped_at: '2026-04-06T14:28:24Z'
section: docs
site: akka-io
source_url: https://doc.akka.io/libraries/akka-core/current/split-brain-resolver.html
title: Split Brain Resolver • Akka core
---

# Split Brain Resolver • Akka core
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-cluster/src/main/resources/reference.conf#L392-L442 "Go to snippet source")  
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-cluster/src/main/resources/reference.conf#L473-L476 "Go to snippet source")akka.cluster.split-brain-resolver.keep-majority {
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-cluster/src/main/resources/reference.conf#L458-L464 "Go to snippet source")akka.cluster.split-brain-resolver.static-quorum {
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-cluster/src/main/resources/reference.conf#L490-L497 "Go to snippet source")akka.cluster.split-brain-resolver.keep-oldest {
...
There is currently one supported implementation of the lease which is backed by a [Custom Resource Definition (CRD)](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) in Kubernetes. It is described in the [Kubernetes Lease](https://doc.akka.io/libraries/akka-management/current/kubernetes-lease.html) documentation.
...
```
[source](https://github.com/akka/akka-core/tree/v2.10.17/akka-cluster/src/main/resources/reference.conf#L505-L521 "Go to snippet source")akka.cluster.split-brain-resolver.lease-majority {
...
See also configuration and additional dependency in [Kubernetes Lease](https://doc.akka.io/libraries/akka-management/current/kubernetes-lease.html)
...
Additional precautions are taken for the uncertainty that may follow from indirectly connected nodes. If indirectly connected decision would down more than a certain fraction of the cluster members, SBR will instead down all nodes. This threshold can be configured with:
...
The value can be `on`, `off`, or a double threshold between 0\.0 and 1\.0: \- `on`: Always down all when there are indirectly connected \- `off`: Disables the safeguard entirely \- A double value: if more or equal to this fraction of members would be downed, down all instead
...
As a precaution for that scenario all nodes are downed if no decision is made within `stable-after + down-all-when-unstable` from the first unreachability event. The measurement is reset if all unreachable have been healed, downed or removed.
...
### Example 4: Enable the Split Brain Resolver

```text
akka.cluster.downing-provider-class = "akka.cluster.sbr.SplitBrainResolverProvider"
```

### Example 5: Stable after
...
### Example 6: Stable after

```text
akka.coordinated-shutdown.exit-jvm = on
```

### Example 7: Keep Majority

```text
akka.cluster.split-brain-resolver.active-strategy=keep-majority
```

### Example 8: Keep Majority
...
### Example 9: Static Quorum

```text
akka.cluster.split-brain-resolver.active-strategy=static-quorum
```

### Example 10: Static Quorum
...
### Example 11: Keep Oldest

```text
akka.cluster.split-brain-resolver.active-strategy=keep-oldest
```

### Example 12: Keep Oldest
...
### Example 15: Down all when indirectly connected
...
### Example 16: Down all when unstable
...
- https://doc.akka.io/docs/akka/current/project/links.html
- https://doc.akka.io/docs/akka/current/remote-security.html
...
---
*Source: [https://doc.akka.io/docs/akka/current/split-brain-resolver.html](https://doc.akka.io/docs/akka/current/split-brain-resolver.html)*