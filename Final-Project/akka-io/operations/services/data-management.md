...
scraped_at: '2026-04-05T19:39:28Z'
section: operations
site: akka-io
source_url: https://doc.akka.io/operations/services/data-management.html
title: 'Data migration :: Akka Documentation'
---

# Data migration :: Akka Documentation
...
Instructions for creating a bucket in AWS can be found [here](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html).

#### Create a user

For the purposes of these instructions, we’re going to call the user `my-akka-user`. Instructions for creating users in AWS can be found [here](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html).
...
Instructions for creating IAM policies in AWS can be found [here](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_create-console.html).

#### Attach IAM policy to user

Having created the policy, you can now attach the policy to the user you created earlier. Instructions for doing that can be found [here](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-attach-detach.html).

#### Create an access key

Now that the user has the right permissions, you need to create an access key for that user. Instructions for doing this can be found [here](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html). Copy the access key ID and secret access key for use in the next step.
...
Instructions for creating a bucket in GCS can be found [here](https://cloud.google.com/storage/docs/creating-buckets).

#### Create a service account

For the purposes of these instructions, we’re going to call the user `my-akka-service-account`. Instructions for creating service accounts in GCP can be found [here](https://cloud.google.com/iam/docs/service-accounts-create).

#### Granting access

For exports, you will need to grant the service account the Storage Object Admin role to the bucket (`roles/storage.objectAdmin`). For imports, you will need to grant the service account the Storage Object Viewer role to the bucket (`roles/storage.objectViewer`). Instructions for managing IAM policies for buckets can be found [here](https://cloud.google.com/storage/docs/access-control/using-iam-permissions).

#### Create the service account key

You will need to create a JSON service account key for the `my-akka-service-account` for Akka to use to access the bucket. This can be done by following [these instructions](https://cloud.google.com/iam/docs/keys-create-delete). Once the key is created, place it in a file called `key.json`.
...
The protobuf schema for the export can be found [here](../_attachments/data_management_schema.proto). While an export can be parsed and formatted as a single protobuf message, for large exports, this may not be practical or possible. Instead, we recommend a streaming approach for reading and writing exports.
...
JSON exports use the protobuf schema specified [here](../_attachments/data_management_schema.proto), with the exception that the JSON is formatted as newline separated JSON.
...
- [`akka service data` commands](../../reference/cli/akka-cli/akka_services_data.html#_see_also)

## Code Examples

### Example 1: Create an IAM policy
...
### Example 2: Create an Akka secret with the access key
...
### Example 3: Create an Akka secret

```bash
akka secrets create generic my-credentials --from-file key.json=path/to/key.json
```

### Example 4: Writing exports as streams
...
### Example 5: Reading exports as streams
...
- https://doc.akka.io/operations/_attachments/data_management_schema.proto
- https://doc.akka.io/reference/cli/akka-cli/akka_services_data.html

---
*Source: [https://doc.akka.io/operations/services/data-management.html](https://doc.akka.io/operations/services/data-management.html)*