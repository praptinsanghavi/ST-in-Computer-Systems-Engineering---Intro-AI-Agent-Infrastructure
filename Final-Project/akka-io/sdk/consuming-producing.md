...
section: sdk
site: akka-io
source_url: https://doc.akka.io/sdk/consuming-producing.html
title: 'Consuming and producing :: Akka Documentation'
---

# Consuming and producing :: Akka Documentation
...
|  | For eventing from an entity inside the same Akka service as the consuming component, use regular [@Consume.FromEventSourcedEntity](#consume-from-event-sourced-entity) instead of Service to Service eventing. |
| --- | --- |
...
[Starting from Snapshot](#_starting_from_snapshot) can be used for service\-to\-service eventing. The snapshot handling is defined on the producer side using `@Produce.ServiceStream`, where the snapshot is transformed to an event that consumers receive like any other event.
...
|  | To guarantee that events for each entity can be read from the message broker in the same order they were written, the cloud event subject id must be specified in metadata along with the event. See how to in [Metadata](#_metadata) below. |
| --- | --- |
...
See [`akka service deploy -h`](../reference/cli/akka-cli/index.html) for details on how to set environment variables when deploying a service.
...
### Example 1: Consume from Event Sourced Entity
...
### Example 2: Starting from Snapshot
...
### Example 3: Consume from Key Value Entity
...
### Example 4: Consume from Workflow
...
### Example 7: Consume from a message broker Topic
...
### Example 9: Producing to a message broker Topic
...
### Example 11: TestKit Mocked Incoming Messages
...
### Example 13: Testing with metadata
...
### Example 14: One Suite, Multiple Tests
...
---
*Source: [https://doc.akka.io/sdk/consuming-producing.html](https://doc.akka.io/sdk/consuming-producing.html)*